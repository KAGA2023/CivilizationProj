...
public:
    ...
    UPROPERTY(VisibleAnywhere, BlueprintReadWrite, Category = "Player Info")
    TSoftObjectPtr<UTexture2D> CountrySmallImg; // 국가 작은 이미지 (슬롯 UI 등에서 사용, 캐시)

    UPROPERTY(VisibleAnywhere, BlueprintReadWrite, Category = "Player Info")
    TSoftObjectPtr<UTexture2D> CountryRoundImg; // 국가 둥근 이미지 (슬롯 UI 등에서 사용, 캐시)
...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "Player Info")
    UTexture2D* GetCountryRoundImg() const { return CountryRoundImg.LoadSynchronous(); }

    UFUNCTION(BlueprintCallable, Category = "Player Info")
    void SetCountryRoundImg(TSoftObjectPtr<UTexture2D> NewCountryRoundImg) { CountryRoundImg = NewCountryRoundImg; }
