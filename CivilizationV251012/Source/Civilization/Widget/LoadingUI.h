...
private:
	...
	UFUNCTION()
	void OnTileSpawnProgress(int32 CurrentCount, int32 TotalCount); // 타일 스폰 진행률 콜백  //체크
	
	UFUNCTION()
	void OnTileSpawnCompleted(); // 타일 스폰 완료 콜백  //체크
	
	void GenerateWorldAsync(); // 비동기 월드 생성 시작
	
	void LoadLevelAsync(); // 비동기 레벨 로딩 시작
	
	void SpawnTilesAsync(); // 비동기 타일 스폰 시작  //체크
