...
FVector UWorldComponent::HexToWorld(FVector2D HexPosition) const
{
    ...
    float x = TILE_SIZE * (FMath::Sqrt(3.0f) * HexPosition.X + FMath::Sqrt(3.0f) / 2.0f * HexPosition.Y);
    float y = TILE_SIZE * (3.0f / 2.0f * HexPosition.Y);
...
{
    // 월드 좌표를 Pointy top 육각형 좌표로 변환
    float q = (FMath::Sqrt(3.0f) / 3.0f * WorldPosition.X - 1.0f / 3.0f * WorldPosition.Y) / TILE_SIZE;
    float r = (2.0f / 3.0f * WorldPosition.Y) / TILE_SIZE;
...
void UWorldComponent::LoadDataTables()
{
    ...
    if (!ClimateDataTable)
    {
        ClimateDataTable = LoadObject<UDataTable>(nullptr, TEXT("/Game/Data/ClimateDataTable.ClimateDataTable"));
        if (!ClimateDataTable)
        {
            UE_LOG(LogTemp, Warning, TEXT("[WorldComponent] ClimateDataTable Load Failed"));
        }
    }
    ...
    {
        LandTypeDataTable = LoadObject<UDataTable>(nullptr, TEXT("/Game/Data/LandTypeDataTable.LandTypeDataTable"));
        if (!LandTypeDataTable)
        {
            UE_LOG(LogTemp, Warning, TEXT("[WorldComponent] LandTypeDataTable Load Failed"));
        }
    }
}
    ...
    for (auto& Pair : HexTiles)
    {
        if (UWorldTile* Tile = Pair.Value)
        {
            if (Tile->GetTerrainType() == ETerrainType::Land)
            {
                ...
                else if (RandomValue < WorldConfig.PlainsPercentage + WorldConfig.HillsPercentage + WorldConfig.ForestPercentage)
                {
                    Tile->SetLandTypeID("Forest");
                }
                else
                {
