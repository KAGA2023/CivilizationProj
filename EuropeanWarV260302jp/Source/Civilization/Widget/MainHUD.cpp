...
void UMainHUD::NativeConstruct()
{
	...
	if (RoundTxt)
	{
		RoundTxt->SetText(FText::FromString(TEXT("1")));
	}
	...
	if (GetWorld())
	{
		...
		GetWorld()->GetTimerManager().SetTimer(RefreshOpenResearchUITimerHandle, this, &UMainHUD::RefreshOpenResearchUI, 0.5f, false);
	}
	...
	if (UnitInfoUIWidget)
	{
		UnitInfoUIWidget->SetVisibility(ESlateVisibility::Hidden);
	}
	...
	if (PlayerWinUIWidget)
	{
		PlayerWinUIWidget->SetVisibility(ESlateVisibility::Hidden);
	}

	// PlayerLoseUI 위젯 초기화 (Hidden으로 설정)
	if (PlayerLoseUIWidget)
	{
		PlayerLoseUIWidget->SetVisibility(ESlateVisibility::Hidden);
	}

	// AIPlayerLoseUI 위젯 초기화 (Hidden으로 설정)
	if (AIPlayerLoseUIWidget)
	{
		AIPlayerLoseUIWidget->SetVisibility(ESlateVisibility::Hidden);
		// 델리게이트 바인딩
...
void UMainHUD::OnTurnChanged(FTurnStruct NewTurn)
{
	...
	if (OtherPlayerTurnUIWidget)
	{
		...
		{
			OtherPlayerTurnUIWidget->SetVisibility(ESlateVisibility::Visible);
		}
	}
}
...
void UMainHUD::OnTileHoverBeginHandler(UWorldTile* Tile)
{
	...
	if (IsInvisible(DiplomacyUIWidget) && IsInvisible(PauseMenuUIWidget) && IsInvisible(OtherPlayerTurnUIWidget)
		&& IsInvisible(PlayerWinUIWidget) && IsInvisible(PlayerLoseUIWidget) && IsInvisible(AIPlayerLoseUIWidget))
	{
...
void UMainHUD::OnSelectedUnitChangedHandler(AUnitCharacterBase* NewSelectedUnit)
{
	...
	{
		UnitInfoUIWidget->SetupForUnit(NewSelectedUnit);
		UnitInfoUIWidget->SetVisibility(ESlateVisibility::Visible);
	}
	else
	{
		CloseUnitInfoUI();
...
void UMainHUD::NativeDestruct()
{
	...
	if (GetWorld())
	{
		...
		GetWorld()->GetTimerManager().ClearTimer(RefreshOpenResearchUITimerHandle);
	}
...
// ========== 승리/패배 시스템 위젯 설정 함수 ==========
void UMainHUD::SetPlayerWinUI(UPlayerWinUI* InPlayerWinUI)
{
	PlayerWinUIWidget = InPlayerWinUI;
...
void UMainHUD::SetPlayerLoseUI(UPlayerLoseUI* InPlayerLoseUI)
{
	PlayerLoseUIWidget = InPlayerLoseUI;
...
void UMainHUD::SetAIPlayerLoseUI(UAIPlayerLoseUI* InAIPlayerLoseUI)
{
	AIPlayerLoseUIWidget = InAIPlayerLoseUI;
...
void UMainHUD::SetOtherPlayerTurnUI(UOtherPlayerTurnUI* InOtherPlayerTurnUI)
{
	OtherPlayerTurnUIWidget = InOtherPlayerTurnUI;
...
void UMainHUD::BindAIPlayerLoseUIDelegate()
{
	...
	{
		// 기존 바인딩 제거 후 새로 바인딩
		AIPlayerLoseUIWidget->OnContinueButtonClickedDelegate.RemoveDynamic(this, &UMainHUD::HideAIPlayerLoseUI);
		AIPlayerLoseUIWidget->OnContinueButtonClickedDelegate.AddDynamic(this, &UMainHUD::HideAIPlayerLoseUI);
	}
}
...
void UMainHUD::ShowVictoryWidget()
{
	...
	if (PlayerWinUIWidget)
	{
		PlayerWinUIWidget->SetVisibility(ESlateVisibility::Visible);
		if (OpenPlayerWinUI)
		{
...
void UMainHUD::ShowDefeatWidget()
{
	...
	if (PlayerLoseUIWidget)
	{
		PlayerLoseUIWidget->SetVisibility(ESlateVisibility::Visible);
		if (OpenPlayerLoseUI)
		{
...
void UMainHUD::ShowAIDefeatWidget(int32 DefeatedPlayerIndex)
{
	...
	if (AIPlayerLoseUIWidget)
	{
		AIPlayerLoseUIWidget->SetupForDefeatedPlayer(DefeatedPlayerIndex);
		AIPlayerLoseUIWidget->SetVisibility(ESlateVisibility::Visible);
