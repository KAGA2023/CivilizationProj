...
FString UMouseUI::GenerateTileInfoString(UWorldTile* Tile)
{
		...
		if (UWorld* World = GetWorld())
		{
			if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(World->GetGameInstance()))
			{
				if (ASuperPlayerState* OwnerState = SuperGameInst->GetPlayerState(Tile->GetOwnerPlayerID()))
				{
					...
					InfoString += CountryName.IsEmpty() ? FString::Printf(TEXT("플레이어%d"), Tile->GetOwnerPlayerID()) : CountryName;
				}
				else
				{
					InfoString += FString::Printf(TEXT("플레이어%d"), Tile->GetOwnerPlayerID());
				}
			}
			else
			{
	...
	switch (ResCat)
	{
	case EResourceCategory::Luxury:    InfoString += TEXT("<사치 자원>\n"); break;
	case EResourceCategory::Bonus:     InfoString += TEXT("<보너스 자원>\n"); break;
	case EResourceCategory::Strategic: InfoString += TEXT("<전략 자원>\n"); break;
	default:                           InfoString += TEXT("<자원 없음>\n"); break;
	}
	...
	InfoString += FString::Printf(TEXT("식량: %d\n"), Food);
	InfoString += FString::Printf(TEXT("생산력: %d\n"), Production);
	InfoString += FString::Printf(TEXT("골드: %d\n"), Gold);
	InfoString += FString::Printf(TEXT("과학력: %d\n"), Science);
