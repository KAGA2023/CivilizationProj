...
void UTechUI::BindToOwnedTilesChanged()
{
	if (!CachedPlayerState)
	{
		return;
	}
	CachedPlayerState->OnOwnedTilesChanged.RemoveDynamic(this, &UTechUI::OnOwnedTilesChanged);
	CachedPlayerState->OnOwnedTilesChanged.AddDynamic(this, &UTechUI::OnOwnedTilesChanged);
}

void UTechUI::OnOwnedTilesChanged()
{
	// 타일 구매 등으로 과학량이 바뀌었을 수 있으므로 TurnTxt 갱신
	UpdateAllSlotsTurnText();
}

void UTechUI::BindToProductionCompleted()
{
	if (!CachedPlayerState)
	{
		return;
	}
	UCityComponent* CityComp = CachedPlayerState->GetCityComponent();
	if (!CityComp)
	{
		return;
	}
	CityComp->OnProductionCompleted.RemoveDynamic(this, &UTechUI::OnProductionCompleted);
	CityComp->OnProductionCompleted.AddDynamic(this, &UTechUI::OnProductionCompleted);
}

void UTechUI::OnProductionCompleted(FName ProductionID)
{
	// 건물 건설 완료 시 과학량이 바뀌었을 수 있으므로 TurnTxt 갱신
	UpdateAllSlotsTurnText();
}
