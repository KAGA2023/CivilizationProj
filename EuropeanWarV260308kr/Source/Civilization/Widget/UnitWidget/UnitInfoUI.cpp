...
void UUnitInfoUI::SetupForUnit(AUnitCharacterBase* Unit)
{
	...
	// UnitMovementPointTxt (현재 턴 남은 이동력 — 해당 턴에 이미 공격했으면 규칙상 이동 불가이므로 0 표기)
	if (UnitMovementPointTxt && GetWorld())
	{
		int32 DisplayMovement = 0;
		if (StatusComp->GetCurrentStat().HasAttacked)
		{
			DisplayMovement = 0;
		}
		else if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(GetWorld()->GetGameInstance()))
		{
			if (UUnitManager* UnitManager = SuperGameInst->GetUnitManager())
			{
				DisplayMovement = UnitManager->GetUnitRemainingMovement(Unit);
			}
		}
		UnitMovementPointTxt->SetText(FText::FromString(FString::Printf(TEXT("%d"), DisplayMovement)));
	}
}
