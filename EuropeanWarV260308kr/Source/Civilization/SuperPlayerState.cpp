...
bool ASuperPlayerState::PurchaseTile(FVector2D TileCoordinate, UWorldComponent* WorldComponent)
{
    ...
    // 소유 타일 변경 알림 (CityProductionUI 등에서 BuildingVB/UnitVB 갱신용)
    OnOwnedTilesChanged.Broadcast();

    // 타일 구매 즉시: 구매한 타일 위에 전쟁/동맹이 아닌 다른 플레이어 유닛이 있으면 쫓아내기
    if (UWorld* World = GetWorld())
    {
        if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(World->GetGameInstance()))
        {
            ...
            {
                if (UDiplomacyManager* DiplomacyManager = SuperGameInst->GetDiplomacyManager())
                {
                    if (AUnitCharacterBase* UnitOnTile = UnitManager->GetUnitAtHex(TileCoordinate))
                    {
                        int32 UnitOwner = UnitOnTile->GetPlayerIndex();
                        if (UnitOwner != PlayerIndex)
                        {
                            EDiplomacyStatusType Status = DiplomacyManager->GetStatus(PlayerIndex, UnitOwner);
                            if (Status != EDiplomacyStatusType::War && Status != EDiplomacyStatusType::Alliance)
                            {
                                UnitManager->EvictUnitToNearestValidTile(UnitOnTile);
                            }
                        }
                    }
                }
    ...
    // 구매 성공 (플레이어 0일 때 Purchase 사운드)
    if (PlayerIndex == 0)
    {
        if (USoundBase* PurchaseSound = LoadObject<USoundBase>(nullptr, TEXT("/Game/Civilization/Sound/UI/Purchase.Purchase")))
        {
...
bool ASuperPlayerState::PurchaseBuildingWithGold(FName BuildingRowName)
{
    ...
    // 플레이어 0 도시 건물 구매 성공 시 Build 사운드
    if (PlayerIndex == 0)
    {
            ...
            if (UUnitManager* UnitManager = SuperGameInst->GetUnitManager())
            {
                ...
                if (SpawnHex != FVector2D(-1, -1))
                {
                    ...
                    if (SpawnedUnit)
                    {
                        ...
                        Population++;
                        OnPopulationChanged.Broadcast(Population);
                        // 플레이어 0(본인) 도시 유닛 구매 성공 시 Build 사운드 재생
                        if (PlayerIndex == 0)
                        {
                            if (USoundBase* BuildSound = LoadObject<USoundBase>(nullptr, TEXT("/Game/Civilization/Sound/Build.Build")))
                            {
...
void ASuperPlayerState::AddLuxuryResource(ELuxuryResource LuxuryResource, int32 Amount)
{
    ...
    OnLuxuryResourceChanged.Broadcast(LuxuryResource, OwnedLuxuryResources.FindChecked(LuxuryResource));
}
...
void ASuperPlayerState::RemoveLuxuryResource(ELuxuryResource LuxuryResource, int32 Amount)
{
    ...
    if (CurrentAmount)
    {
        if (*CurrentAmount <= Amount)
        {
            ...
            UpdatePopulationLimitFromLuxury();
            OnLuxuryResourceChanged.Broadcast(LuxuryResource, 0);
        }
        else
        {
            // 일부만 제거
            *CurrentAmount -= Amount;
            UpdatePopulationLimitFromLuxury();
            OnLuxuryResourceChanged.Broadcast(LuxuryResource, *CurrentAmount);
        }
    }
}
