...
void ASuperGameModeBase::BeginPlay()
{
	...
	// PostLogin이 BeginPlay보다 먼저 올 수 있어 PlayerState0 연결이 안 됐을 수 있음 → 여기서 한 번 더 연결
...
void ASuperGameModeBase::CreateAllPlayerStates()
{
	...
	if (TotalPlayerCount < 2) TotalPlayerCount = 2;
	else if (TotalPlayerCount > 8) TotalPlayerCount = 8;
	
	TArray<FName> GameInstanceCountryNames = GameInstance->GetCountryNames();
