...
#include "../Combat/RangedProjectileVisual.h"
...
void UUnitVisualizationComponent::UpdateCombatVisualization(float DeltaTime)
{
    ...
    switch (CombatState)
    {
        ...
        {
            // 공격자 또는 방어자 복귀 중 (근거리) — StartCombatMovement 사용 → bIsCombatMoving으로 완료 판정
            if (!bIsCombatMoving)
            {
                // 복귀 완료 처리
                // 공격자인지 방어자인지 확인
...
void UUnitVisualizationComponent::StartMovingToDefender()
{
    ...
    // 방어자로부터 60.0f 떨어진 목표 위치 계산
    FVector TargetWorldPosition = DefenderWorldPos + DirectionToAttacker * 60.0f;
...
void UUnitVisualizationComponent::PlayDefenderHitOrDeathMontage()
{
    ...
    // 방어자가 죽었으면 사망 몽타주, 아니면 피격 몽타주 (DeathSound는 OnCombatNotify_Hit에서 이미 재생)
    ...
    if (UUnitVisualizationComponent* DefenderVisComp = Defender->GetUnitVisualizationComponent())
    {
        DefenderVisComp->bIsHitOnlyMontage = false;
    }
    if (!CurrentCombatResult.bDefenderAlive)
    {
...
void UUnitVisualizationComponent::PlayDefenderHitMontageOnly()
{
    if (!CombatDefender.IsValid())
    {
        return;
    ...
    if (UUnitVisualizationComponent* DefenderVisComp = Defender->GetUnitVisualizationComponent())
    {
        DefenderVisComp->bIsHitOnlyMontage = true;
        DefenderVisComp->CombatState = ECombatVisualizationState::DefenderHit;
        DefenderVisComp->PlayMontage(MontageToPlay, Defender);
    }
}

void UUnitVisualizationComponent::PlayDefenderHitMontageOnly_Ranged()
{
    if (!CombatDefender.IsValid())
    {
        return;
...
void UUnitVisualizationComponent::PlayBuilderActionMontage(FSimpleDelegate InOnCompleted)
{
    AUnitCharacterBase* Builder = Cast<AUnitCharacterBase>(GetOwner());
    if (!Builder)
    {
    ...
    UAnimMontage* MontageToPlay = nullptr;
    if (const FUnitData* UnitData = Builder->GetUnitData())
    {
        MontageToPlay = UnitData->AttackMontage;
    }
    if (!MontageToPlay)
    ...
    // 몽타주 종료 시 이 컴포넌트의 OnMontageEnded가 호출되도록 바인딩
    if (USkeletalMeshComponent* MeshComp = Builder->GetMesh())
    {
        if (UAnimInstance* AnimInstance = MeshComp->GetAnimInstance())
        {
            AnimInstance->OnMontageEnded.RemoveAll(this);
    ...
    bIsPlayingBuilderAction = true;
    OnBuilderActionMontageCompleted = InOnCompleted;
    PlayMontage(MontageToPlay, Builder);
}
...
void UUnitVisualizationComponent::OnMontageEnded(UAnimMontage* Montage, bool bInterrupted)
{
    ...
    // 건설자 액션(건설/수리/제거) 몽타주 종료 시 콜백만 실행 후 return
    if (bIsPlayingBuilderAction)
    {
        bIsPlayingBuilderAction = false;
        FSimpleDelegate Callback = OnBuilderActionMontageCompleted;
        OnBuilderActionMontageCompleted.Unbind();
...
{
    // 도시 공격: Notify_Combat 타이밍에 공격 사운드 + 투사체 발사 + Destroy 사운드 재생
    if (CombatState == ECombatVisualizationState::AttackingCity)
    {
        AUnitCharacterBase* CityAttacker = Cast<AUnitCharacterBase>(GetOwner());
        if (CityAttacker && CityAttacker->GetUnitData() && CityAttacker->GetUnitData()->AttackSound)
        {
            UGameplayStatics::PlaySound2D(this, CityAttacker->GetUnitData()->AttackSound);
        }
        SpawnRangedProjectile();
        if (!CityAttackDestroySound)
        {
            CityAttackDestroySound = LoadObject<USoundBase>(nullptr, TEXT("/Game/Civilization/Sound/Destroy.Destroy"));
        }
        if (CityAttackDestroySound)
        {
            UGameplayStatics::PlaySound2D(this, CityAttackDestroySound);
        }
        return;
    }

    // Notify_Combat 타이밍에 원거리 화살 파티클 발사
    if (CombatState == ECombatVisualizationState::AttackerAttack_Ranged)
    {
        SpawnRangedProjectile();
    }

    // Notify_Combat: 생존 시 AttackSound + HitSound, 사망 시 AttackSound + DeathSound
    AUnitCharacterBase* CurrentAttacker = Cast<AUnitCharacterBase>(GetOwner());
    AUnitCharacterBase* CurrentDefender = (CombatState == ECombatVisualizationState::DefenderCounter && CombatAttacker.IsValid())
        ? CombatAttacker.Get()
        : (CombatDefender.IsValid() ? CombatDefender.Get() : nullptr);
    const bool bIsKillingBlow = (CombatState == ECombatVisualizationState::AttackerAttack || CombatState == ECombatVisualizationState::AttackerAttack_Ranged)
        && !CurrentCombatResult.bDefenderAlive;
    if (CurrentAttacker && CurrentDefender)
    {
        if (const FUnitData* AttackerData = CurrentAttacker->GetUnitData())
        {
            if (AttackerData->AttackSound)
            {
                UGameplayStatics::PlaySound2D(this, AttackerData->AttackSound);
            }
        }
        if (const FUnitData* DefenderData = CurrentDefender->GetUnitData())
        {
            if (bIsKillingBlow)
            {
                if (DefenderData->DeathSound)
                {
                    UGameplayStatics::PlaySound2D(this, DefenderData->DeathSound);
                }
            }
            else
            {
                if (DefenderData->HitSound)
                {
                    UGameplayStatics::PlaySound2D(this, DefenderData->HitSound);
                }
            }
        }
    }
...
void UUnitVisualizationComponent::OnCombatNotify_HitOnly()
{
    // 원거리/도시 공격: Notify_Hit 시 공격 사운드 + 투사체만 발사 (방어자 Hit·몽타주 없음)
    if (CombatState == ECombatVisualizationState::AttackerAttack_Ranged || CombatState == ECombatVisualizationState::AttackingCity)
    {
        AUnitCharacterBase* CurrentAttacker = Cast<AUnitCharacterBase>(GetOwner());
        if (CurrentAttacker && CurrentAttacker->GetUnitData() && CurrentAttacker->GetUnitData()->AttackSound)
        {
            UGameplayStatics::PlaySound2D(this, CurrentAttacker->GetUnitData()->AttackSound);
        }
        SpawnRangedProjectile();
        return;
    }

    // Notify_Hit(연타): 공격 사운드만 재생 (HitSound는 연달아 들려서 제외)
    AUnitCharacterBase* CurrentAttacker = Cast<AUnitCharacterBase>(GetOwner());
    if (CurrentAttacker)
    {
        if (const FUnitData* AttackerData = CurrentAttacker->GetUnitData())
    ...
    // Notify_Hit: Hit 몽타주만 재생 (Death 없음). 연타 연출용.
    // AttackerAttack_Ranged / AttackingCity 는 위에서 투사체만 발사 후 return 함
    switch (CombatState)
    {
        case ECombatVisualizationState::AttackerAttack:
            PlayDefenderHitMontageOnly();
            break;
        case ECombatVisualizationState::AttackerAttack_Ranged:
        case ECombatVisualizationState::AttackingCity:
            // 위에서 이미 SpawnRangedProjectile() 후 return 하므로 여기 도달하지 않음 (명시용)
            break;
        case ECombatVisualizationState::DefenderCounter:
            if (CombatAttacker.IsValid())
            {
...
void UUnitVisualizationComponent::SpawnRangedProjectile()
{
    AUnitCharacterBase* Attacker = Cast<AUnitCharacterBase>(GetOwner());
    if (!Attacker)
    {
    ...
    UStaticMeshComponent* WeaponMesh = Attacker->GetWeaponMeshComponent();
    if (!WeaponMesh)
    {
        return;
    }

    static const FName SocketName(TEXT("SFXsocket"));
    FVector StartLoc = WeaponMesh->DoesSocketExist(SocketName)
        ? WeaponMesh->GetSocketLocation(SocketName)
    ...
    {
        AUnitCharacterBase* Defender = CombatDefender.Get();
        EndLoc = Defender->GetMesh()
            ? Defender->GetMesh()->GetComponentLocation()
            : Defender->GetActorLocation();
    }
    else if (bIsCityCombat && WorldComponent)
    {
        // 도시 공격: 도시 액터 위치를 목표로 하여 투사체가 도시에 가까워지면 사라지도록 함
        UWorld* World = GetWorld();
        if (World)
        {
            if (AWorldSpawner* WorldSpawner = Cast<AWorldSpawner>(UGameplayStatics::GetActorOfClass(World, AWorldSpawner::StaticClass())))
            {
                const FVector2D RoundedHex(FMath::RoundToInt(CityHexPosition.X), FMath::RoundToInt(CityHexPosition.Y));
                if (ACityActor* CityActor = WorldSpawner->GetCityActorAtHex(RoundedHex))
                {
                    EndLoc = CityActor->GetActorLocation();
                }
                else
                {
                    EndLoc = CityWorldPosition;
                }
            }
            else
            {
    ...
    UClass* ProjectileClass = (RangedProjectileClass != nullptr) ? RangedProjectileClass.Get() : ARangedProjectileVisual::StaticClass();
    FActorSpawnParameters SpawnParams;
    SpawnParams.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;

    if (ARangedProjectileVisual* Projectile = World->SpawnActor<ARangedProjectileVisual>(ProjectileClass, StartLoc, FRotator::ZeroRotator, SpawnParams))
    {
        Projectile->InitProjectile(StartLoc, EndLoc, RangedProjectileSpeed);
    }
}

void UUnitVisualizationComponent::OnDefenderHitOrDeathMontageEnded()
{
    // 중간 Hit 전용 몽타주가 끝난 경우 다음 단계로 진행하지 않음
    if (bIsHitOnlyMontage)
    {
        bIsHitOnlyMontage = false;
        return;
    }
