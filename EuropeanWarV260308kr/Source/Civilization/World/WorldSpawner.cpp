...
{
	// BeginPlay보다 NativeConstruct가 먼저 올 수 있음 → WorldComponent가 아직 null일 때 GI에서 한 번 더 조회
...
void AWorldSpawner::AssignCitiesToPlayers()
{
	...
	for (int32 i = 0; i < CityHexes.Num(); i++)
	{
		...
		// 도시 머리 위 UI 초기값 설정 (국가 이름/이미지, 체력바)
		if (UCityComponent* CityComp = PlayerState->GetCityComponent())
		{
			City->UpdateCitySmogVisibility(CityComp->GetCurrentHealth(), CityComp->GetMaxHealth());
			City->UpdateSmallCityUI(PlayerState->GetCountryName(), PlayerState->GetCountryLargeImg(), CityComp->GetCurrentHealth(), CityComp->GetMaxHealth());
		}
...
void AWorldSpawner::RestoreGameStateFromSave(const FGameSaveData& SaveData)
{
	...
	// GameInstance의 국가 목록을 세이브 데이터로 맞춤 (다른 시스템이 GetCountryNames()를 참조할 때 일치하도록)
	...
	SuperGameInst->SetCountryNames(RestoredCountryNames);
