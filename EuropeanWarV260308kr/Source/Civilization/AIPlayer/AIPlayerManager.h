...
public:
	...
	// 다음 턴 전환 1초 대기 후 호출 (재귀 Broadcast 방지)
	UFUNCTION()
...
protected:
	...
	// 갇힌 AI 유닛 제거 (데이터·월드에서 Destroy, 인구 반환) — AI 턴 시작 시 호출
	...
	// 배회 타일 찾기
	// 여러 번 시도하여 유닛이 실제로 이동 가능한(경로 있는) 배회 타일을 찾음
	// @param PlayerIndex AI 플레이어 인덱스
	// @param Radius 도시 기준 반경 (기본값: 2)
	// @param MaxAttempts 최대 시도 횟수 (기본값: 10)
	// @param Unit 배회할 유닛 (null이면 경로 검사 없이 랜덤 타일 반환)
	// @return 이동 가능한 배회 타일 좌표, 실패 시 FVector2D(-1, -1)
