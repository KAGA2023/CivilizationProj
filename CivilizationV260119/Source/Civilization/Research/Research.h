...
struct CIVILIZATION_API FTechData : public FTableRowBase
{
    ...
    TArray<FName> RequiredTechs;                                         // 선행 기술들 (RowName 배열)

    // 해제되는 시설
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Unlocks")
    TArray<FName> UnlockFacilities;                                         // 해제되는 시설 목록 (RowName 배열)

    // 해제되는 건물
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Unlocks")
    TArray<FName> UnlockBuildings;                                         // 해제되는 건물 목록 (RowName 배열)

    // 해제되는 유닛
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Unlocks")
    TArray<FName> UnlockUnits;                                             // 해제되는 유닛 목록 (RowName 배열)
...
struct CIVILIZATION_API FResearchData
{
    ...
    TArray<FName> ResearchedTechs;                                           // 연구 완료된 기술들 (RowName으로 저장)
