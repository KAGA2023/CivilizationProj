...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "World Generation")
    void GenerateWorldFromSaveData(const TMap<FVector2D, struct FWorldSaveData>& WorldDataMap, const FWorldConfig& Config, const TArray<struct FPlayerSaveData>& PlayerDataArray); // 저장된 데이터로부터 월드 생성
	...
	// NumCities는 WorldConfig.PlayerCount를 사용하는 것을 권장
