...
void UWorldComponent::GenerateWorld()
{
    ...
    // 도시 시작 위치 계산 (데이터 전용, WorldConfig.PlayerCount 사용)
    GenerateCities(WorldConfig.PlayerCount);
...
void UWorldComponent::GenerateWorldFromSaveData(const TMap<FVector2D, FWorldSaveData>& WorldDataMap, const FWorldConfig& Config, const TArray<FPlayerSaveData>& PlayerDataArray)
{
    bIsWorldGenerated = false;
    ...
    // StartingCityHexes 초기화 및 저장된 도시 좌표 복원
    StartingCityHexes.Empty();
    for (const FPlayerSaveData& PlayerData : PlayerDataArray)
    {
        if (PlayerData.bHasCity && PlayerData.CityData.CityCoordinate != FVector2D::ZeroVector)
        {
            StartingCityHexes.Add(PlayerData.CityData.CityCoordinate);
        }
    }
    ...
    {
        const FVector2D& HexPosition = Pair.Key;
        const FWorldSaveData& WorldData = Pair.Value;
        
        // 타일 생성
        UWorldTile* Tile = CreateTileAtHex(HexPosition);
        if (!Tile)
        {
            continue; // 타일 생성 실패 시 스킵
        }
