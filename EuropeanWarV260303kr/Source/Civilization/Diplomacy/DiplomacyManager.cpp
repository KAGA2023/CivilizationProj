...
void UDiplomacyManager::Initialize(int32 NumPlayers)
{
	...
	for (int32 PlayerA = 0; PlayerA < NumPlayers; ++PlayerA)
	{
		for (int32 PlayerB = PlayerA + 1; PlayerB < NumPlayers; ++PlayerB)
		{
			...
			PairState.LastOfferPeaceRound = 0;
			PairState.LastOfferAllianceRound = 0;
...
bool UDiplomacyManager::DeclareWar(int32 PlayerA, int32 PlayerB, int32 CurrentRound)
{
	...
	if (!State)
	{
		...
		NewState.LastOfferPeaceRound = 0;
		NewState.LastOfferAllianceRound = 0;
...
int32 UDiplomacyManager::IssueAction(const FDiplomacyAction& Action)
{
	...
	// ========== 생존 확인 (패배한 플레이어와는 외교 불가) ==========
	if (UWorld* World = GetWorld())
	{
		if (USuperGameInstance* GameInstance = Cast<USuperGameInstance>(World->GetGameInstance()))
		{
			ASuperPlayerState* FromState = GameInstance->GetPlayerState(Action.FromPlayerId);
			ASuperPlayerState* ToState = GameInstance->GetPlayerState(Action.ToPlayerId);
			
			if (!FromState || !FromState->IsAlive() || !ToState || !ToState->IsAlive())
			{
				return -1; // 패배한 플레이어와는 외교 불가
			}
		}
	}
	...
	case EDiplomacyActionType::DeclareWar:
	{
		...
		AddAttitude(Action.FromPlayerId, Action.ToPlayerId, -30);
		break;
	}
	...
	case EDiplomacyActionType::OfferPeace:
	{
		...
		// 마지막 평화 제안 후 5라운드가 지나지 않았으면 불가
		if (State->LastOfferPeaceRound > 0 && (CachedCurrentRound - State->LastOfferPeaceRound) < 5)
		{
			return -1;
		}
		// 평화 제안 라운드 기록
		if (State)
		{
			State->LastOfferPeaceRound = CachedCurrentRound;
		}
		// 큐에 추가 (허락 필요)
		break;
	}
	...
	case EDiplomacyActionType::Denounce:
	{
		...
		if (State && State->LastDenounceRound > 0 && (CachedCurrentRound - State->LastDenounceRound) < 3)
		{
			return -1;
		}
		// 즉시 실행
	...
	case EDiplomacyActionType::SendGift:
	{
		...
		if (State && State->LastGiftRound > 0 && (CachedCurrentRound - State->LastGiftRound) < 3)
		{
			return -1;
		}
		// 즉시 실행
	...
	case EDiplomacyActionType::OfferAlliance:
	{
		...
		if (State && State->LastOfferAllianceRound > 0 && (CachedCurrentRound - State->LastOfferAllianceRound) < 5)
		{
			return -1;
		}
		// 동맹 제안 라운드 기록
		if (State)
		{
			State->LastOfferAllianceRound = CachedCurrentRound;
		}
		// 큐에 추가 (허락 필요)
		break;
	}
		...
		Action.Action == EDiplomacyActionType::SendGift)
	{
		...
		if (bPlayer0Involved)
		{
			...
			{
				if (USoundBase* WarSound = LoadObject<USoundBase>(nullptr, TEXT("/Game/Civilization/Sound/War.War")))
				{
					UGameplayStatics::PlaySound2D(this, WarSound);
				}
			}
			else if (NewAction.Action == EDiplomacyActionType::Denounce || NewAction.Action == EDiplomacyActionType::SendGift)
			{
				if (USoundBase* LightDiplocySound = LoadObject<USoundBase>(nullptr, TEXT("/Game/Civilization/Sound/LightDiplocy.LightDiplocy")))
				{
					UGameplayStatics::PlaySound2D(this, LightDiplocySound);
				}
			}
		}
	...
	case EDiplomacyActionType::OfferPeace:
	{
		...
		if (bAccepted)
		{
			...
			if (Action.FromPlayerId == 0 || Action.ToPlayerId == 0)
			{
				...
				{
					UGameplayStatics::PlaySound2D(this, HeavyDiplocySound);
				}
			}
		}
		break;
...
void UDiplomacyManager::CleanupPlayerDiplomacy(int32 DefeatedPlayerId)
{
	// 인덱스 범위 검증
	if (DefeatedPlayerId < 0 || DefeatedPlayerId >= PlayerCount)
	{
		return;
	}

	// 1. 모든 외교 관계 종료 (PairStates에서 패배한 플레이어가 포함된 관계를 None으로 변경)
	for (TPair<FDiplomacyPairKey, FDiplomacyPairState>& Pair : PairStates)
	{
		const FDiplomacyPairKey& Key = Pair.Key;
		FDiplomacyPairState& State = Pair.Value;

		if (Key.PlayerA == DefeatedPlayerId || Key.PlayerB == DefeatedPlayerId)
		{
			// 기존 상태가 None이 아니면 델리게이트 브로드캐스트 (UI 업데이트용)
			if (State.Status != EDiplomacyStatusType::None)
			{
				State.Status = EDiplomacyStatusType::None;
				// 패배로 인한 외교 관계 종료는 조용히 처리 (델리게이트 브로드캐스트 안 함)
				// OnDiplomacyStatusChanged.Broadcast(Key.PlayerA, Key.PlayerB, EDiplomacyStatusType::None);
			}
		}
	}

	// 2. 대기 중인 외교 액션 모두 제거 (FromPlayerId 또는 ToPlayerId가 패배한 플레이어인 액션)
	PendingActions.RemoveAll([DefeatedPlayerId](const FDiplomacyAction& Action)
	{
		return Action.FromPlayerId == DefeatedPlayerId || Action.ToPlayerId == DefeatedPlayerId;
	});

	// 3. 호감도 초기화 (패배한 플레이어 관련 호감도 제거)
	// From 제거 (패배한 플레이어가 다른 플레이어에게 가진 호감도)
	Attitudes.Remove(DefeatedPlayerId);

	// To 제거 (다른 플레이어가 패배한 플레이어에게 가진 호감도)
	for (TPair<int32, TMap<int32, int32>>& FromPair : Attitudes)
	{
		FromPair.Value.Remove(DefeatedPlayerId);
	}
}