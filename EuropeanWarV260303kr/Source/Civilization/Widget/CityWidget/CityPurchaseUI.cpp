...
void UCityPurchaseUI::CreateBuildingSlots(const TArray<FName>& BuildingNames)
{
	...
	for (const FName& BuildingName : BuildingNames)
	{
		...
		if (PurchaseSlot)
		{
			// PurchaseID 설정 및 건물 슬롯 플래그
			PurchaseSlot->PurchaseID = BuildingName;
			PurchaseSlot->bIsBuildingSlot = true;
			...
			PurchaseSlot->OnPurchaseSlotHovered.AddDynamic(this, &UCityPurchaseUI::OnPurchaseSlotHovered);
			PurchaseSlot->OnPurchaseSlotUnhovered.AddDynamic(this, &UCityPurchaseUI::OnPurchaseSlotUnhovered);
...
void UCityPurchaseUI::CreateUnitSlots(const TArray<FName>& UnitNames)
{
	...
	for (const FName& UnitName : UnitNames)
	{
		...
		if (PurchaseSlot)
		{
			// PurchaseID 설정 및 유닛 슬롯 플래그
			PurchaseSlot->PurchaseID = UnitName;
			PurchaseSlot->bIsBuildingSlot = false;
...
void UCityPurchaseUI::OnPurchaseSlotHovered(FName PurchaseID, bool bIsBuilding)
{
	if (!CachedCityComponent || PurchaseID == NAME_None)
	{
		return;
	}
	...
	if (bIsBuilding)
	{
		FBuildingData BuildingData = CachedCityComponent->GetBuildingDataFromTable(PurchaseID);
	...
	{
		FUnitBaseStat UnitData = CachedCityComponent->GetUnitDataFromTable(PurchaseID);
...
void UCityPurchaseUI::OnPurchaseSlotUnhovered()
{
	if (BuildingSlotInfoWidget)
	{
		BuildingSlotInfoWidget->SetVisibility(ESlateVisibility::Hidden);
