...
// 선택 유닛 변경 이벤트 델리게이트 (nullptr = 선택 해제)
...
public:
    ...
    // 타국 영토 쫓아내기: 배치 가능한 가장 가까운 타일로 순간이동 (타일 구매/전쟁·동맹 종료 시)
    UFUNCTION(BlueprintCallable, Category = "Unit Management")
    ...
    // 갇힌 유닛 판정: 인접 6타일 중 한 칸이라도 이동 가능하면 false, 전부 이동 불가면 true (1층 규칙·바다 반영)
    UFUNCTION(BlueprintCallable, Category = "Unit Management")
    ...
    // 건설자 시설 액션 (공격 몽타주 1회 재생 후 실제 실행). 타일에 건설자 없으면 몽타주 없이 바로 실행
    UFUNCTION(BlueprintCallable, Category = "Unit Management")
    void RequestBuilderBuildFacility(FVector2D Hex, FName FacilityRowName); // 건설 → 몽타주 종료 후 BuildFacility + 건설자 제거
    UFUNCTION(BlueprintCallable, Category = "Unit Management")
    void RequestBuilderRepairFacility(FVector2D Hex); // 수리 → 몽타주 종료 후 RepairFacility
    UFUNCTION(BlueprintCallable, Category = "Unit Management")
    void RequestBuilderDestroyFacility(FVector2D Hex); // 시설 제거 → 몽타주 종료 후 DestroyFacility
    ...
    void StartMovementImmediate(class AUnitCharacterBase* Unit, const TArray<FVector2D>& Path); // 시각화 없이 즉시 이동 (AI용)
    ...
    TArray<FVector2D> FindPath(FVector2D StartHex, FVector2D EndHex, int32 MoverPlayerIndex = 0) const; // A* 알고리즘을 사용한 최적 경로 찾기 (국경선 적용)

    UFUNCTION(BlueprintCallable, Category = "Pathfinding")
    TArray<FVector2D> FindPathWithMovementCost(FVector2D StartHex, FVector2D EndHex, int32 MaxMovementCost, int32 MoverPlayerIndex = 0) const; // 최대 이동 비용 제한이 있는 경로 찾기

    UFUNCTION(BlueprintCallable, Category = "Pathfinding")
    bool CanMoveToHex(FVector2D HexPosition, int32 MoverPlayerIndex = 0) const; // 해당 육각형으로 이동 가능한지 확인 (본인/무주 타일 또는 전쟁·동맹국 타일만)
    ...
    UFUNCTION(BlueprintCallable, Category = "Pathfinding")
    int32 GetMovementCostBetweenHexes(FVector2D FromHex, FVector2D ToHex, int32 MoverPlayerIndex = 0) const; // 두 육각형 간의 이동 비용 계산
    ...
    UFUNCTION(BlueprintCallable, Category = "Floor System")
    bool CanMoveBetweenHexes(FVector2D FromHex, FVector2D ToHex, int32 MoverPlayerIndex = 0) const; // 두 육각형 간 층수·국경선 이동 가능성 체크

    UFUNCTION(BlueprintCallable, Category = "Floor System")
    int32 GetMovementCostBetweenHexesWithFloor(FVector2D FromHex, FVector2D ToHex, int32 MoverPlayerIndex = 0) const; // 층수를 고려한 이동 비용 계산
    ...
    // 선택 유닛 변경 이벤트 (MainHUD 등에서 UnitInfoUI 표시/숨김용)
...
private:
    ...
    // 국경선: 플레이어가 해당 타일에 들어갈 수 있는지 (소유 없음 / 본인 소유 / 전쟁 또는 동맹국 소유)
    bool CanPlayerEnterTile(FVector2D HexPosition, int32 MoverPlayerIndex) const;

    // 쫓아내기: FromHex에서 시작해 유닛을 배치 가능한 가장 가까운 hex 탐색 (BFS)
