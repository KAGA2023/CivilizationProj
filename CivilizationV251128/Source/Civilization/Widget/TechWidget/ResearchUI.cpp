...
void UResearchUI::UpdateResearchInfo()
{
	...
	// 기술 데이터 가져오기
	FTechData TechData = CachedTechComponent->GetTechDataFromTable(CurrentStat.DevelopingName);
	...
	if (DevelopingBar)
	{
		...
		if (CurrentStat.DevelopingCost > 0)
		{
...
void UResearchUI::OnTechResearchStarted(FName TechRowName)
{
	// 연구 시작 시 연구 정보 업데이트
	UpdateResearchInfo();
}

void UResearchUI::OnTechResearchCompleted(FName TechRowName)
{
	// 연구 완료 시 연구 정보 업데이트 (연구가 완료되어 DevelopingName이 None이 됨)
	UpdateResearchInfo();
}
