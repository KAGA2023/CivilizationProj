...
void ASuperPlayerState::ProcessTurnResources()
{
    ...
    if (TechComponent)
    {
        ...
        TechComponent->UpdateTechResearchProgress(TurnScience);
    }
...
// ========== 기술 관리 함수들 ==========
void ASuperPlayerState::SetTechComponent(UTechComponent* InTechComponent)
{
    TechComponent = InTechComponent;
    
    if (TechComponent)
    {
        // TechComponent 초기화 (PlayerState 참조 설정)
        TechComponent->InitializeTechSystem(this);
    }
}
...
bool ASuperPlayerState::StartTechResearch(FName TechRowName)
{
    ...
    // 기술 연구 시작/변경 (TechComponent에서 같은 기술이면 무시, 다른 연구 상태도 자동 리셋)
    TechComponent->StartTechResearch(TechRowName);
    return true;
}
...
{
    // 이미 데이터가 로드되어 있고 AvailableFacilities가 비어있지 않으면 스킵
    if (FacilityDataTable && AvailableFacilities.Num() > 0)
    {
        return;
    }
    ...
    // AvailableFacilities 비우기 (데이터 테이블이 로드된 후)
    AvailableFacilities.Empty();
    ...
    // 플레이어 보유 기술 확인하여 필터링
    // TODO: TechComponent에서 연구 완료된 기술들의 UnlockedFacilities 확인
    // 현재는 모든 시설을 추가 (기술 시스템 연동 전)
    for (const FName& RowName : AllRowNames)
    {
        ...
        // 기술 조건 확인
        // TODO: TechComponent가 있고 연구 완료된 기술들의 UnlockedFacilities에 포함되어 있는지 확인
        // if (TechComponent)
        // {
        //     TArray<FName> ResearchedTechs = TechComponent->GetResearchedTechs();
        //     for (const FName& TechRowName : ResearchedTechs)
        //     {
        //         FTechData TechData = TechComponent->GetTechDataFromTable(TechRowName);
        //         if (TechData.UnlockedFacilities.Contains(RowName))
        //         {
