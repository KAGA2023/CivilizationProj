...
void UWorldSettingMenuUI::OnPlayerCountSliderValueChanged(float Value)
{
	...
	// CountrySelectBrd 리셋 애니메이션
	if (ResetCountrySelectBrd)
	{
		PlayAnimation(ResetCountrySelectBrd, 0.f, 1, EUMGSequencePlayMode::Forward, 1.f);
	}
...
void UWorldSettingMenuUI::InitializeSlotWithNoSelect(UCountrySelectSlotUI* TargetSlot)
{
	...
	{
		UTexture2D* CountryImage = CountryData->CountryLargeImg.LoadSynchronous();
		if (CountryImage)
		{
...
void UWorldSettingMenuUI::InitializeCountrySelectMiniSlots()
{
	...
	// NoSelect 이미지 가져오기 (CountryLargeImg 사용)
	UTexture2D* NoSelectImage = nullptr;
	if (!NoSelectData->CountryLargeImg.IsNull())
	{
		NoSelectImage = NoSelectData->CountryLargeImg.LoadSynchronous();
	}
	...
	for (int32 i = 0; i < TotalSlots && i < CountryRowNames.Num(); i++)
	{
		...
		if (CountryData && !CountryData->CountryLargeImg.IsNull())
		{
			CountryImages[i] = CountryData->CountryLargeImg.LoadSynchronous();
		}
		else
		{
...
void UWorldSettingMenuUI::InitializeHoveredCountryUI()
{
	...
	// HoveredCountryImg 설정 (CountryKingImg 사용)
	if (HoveredCountryImg)
	{
		if (!NoSelectData->CountryKingImg.IsNull())
		{
			UTexture2D* NoSelectImage = NoSelectData->CountryKingImg.LoadSynchronous();
			if (NoSelectImage)
			{
...
void UWorldSettingMenuUI::OnCountrySelectMiniSlotHovered(UCountrySelectMiniSlot* HoveredSlot)
{
	...
	if (HoveredCountryImg)
	{
		if (!CountryData->CountryKingImg.IsNull())
		{
			UTexture2D* CountryImage = CountryData->CountryKingImg.LoadSynchronous();
			if (CountryImage)
			{
