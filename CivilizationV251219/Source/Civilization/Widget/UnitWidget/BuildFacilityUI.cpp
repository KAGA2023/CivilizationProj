...
void UBuildFacilityUI::NativeConstruct()
{
	...
	if (BuildChurchBtn)
	{
		BuildChurchBtn->OnClicked.AddDynamic(this, &UBuildFacilityUI::OnClickedBuildChurch);
	}
	if (BuildSchoolBtn)
	{
...
void UBuildFacilityUI::OnClickedBuildChurch()
{
	BuildFacilityByRowName(TEXT("Church"));
}
...
void UBuildFacilityUI::UpdateButtonStates(const TArray<FName>& AvailableFacilities)
{
	...
	if (!CachedTile || !CachedFacilityManager || !CachedPlayerState)
	{
		...
		if (BuildChurchBtn)      BuildChurchBtn->SetIsEnabled(false);
	...
	{
		bool bEnable = false;
		if (AvailableFacilities.Contains(TEXT("Church")))
		{
			bEnable = CachedFacilityManager->CanBuildFacilityOnTile(TEXT("Church"), CachedTile);
		}
		BuildChurchBtn->SetIsEnabled(bEnable);
	}
