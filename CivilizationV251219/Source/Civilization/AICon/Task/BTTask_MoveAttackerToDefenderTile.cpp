...
EBTNodeResult::Type UBTTask_MoveAttackerToDefenderTile::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
    ...
    // 공격자의 시작 타일 저장 (점프 조건 체크를 위해)
    UWorldComponent* WorldComponent = UnitAIController->GetWorldComponent();
    if (WorldComponent)
    {
        FVector CurrentPosition = Pawn->GetActorLocation();
        FVector2D HexPos = WorldComponent->WorldToHex(CurrentPosition);
        // 헥스 좌표를 정수로 반올림 (GetTileAtHex가 정확한 키 매칭을 위해 필요)
        AttackerStartHex = FVector2D(FMath::RoundToInt(HexPos.X), FMath::RoundToInt(HexPos.Y));
    }
...
void UBTTask_MoveAttackerToDefenderTile::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
    ...
    FVector2D DefenderHexFloat = WorldComponent->WorldToHex(Defender->GetActorLocation());
    // 헥스 좌표를 정수로 반올림 (GetTileAtHex가 정확한 키 매칭을 위해 필요)
    FVector2D DefenderHex = FVector2D(FMath::RoundToInt(DefenderHexFloat.X), FMath::RoundToInt(DefenderHexFloat.Y));
    ...
    if (Distance <= 300.0f && LastJumpedTargetHex != DefenderHex)
    {
        ...
        // 시작 타일과 방어자 타일을 비교 (이동 중 AttackerHex가 변해도 시작 타일 기준으로 점프 조건 체크)
        UWorldTile* AttackerStartTile = WorldComponent->GetTileAtHex(AttackerStartHex);
        UWorldTile* DefenderTile = WorldComponent->GetTileAtHex(DefenderHex);
        
        if (AttackerStartTile && DefenderTile)
        {
            ELandType AttackerStartLandType = AttackerStartTile->GetLandType();
            ...
            if (AttackerStartLandType == ELandType::Plains && DefenderLandType == ELandType::Hills)
            {
                bShouldJump = true;
            }
            else if (AttackerStartLandType == ELandType::Hills && DefenderLandType == ELandType::Mountains)
            {
                bShouldJump = true;
            }
    ...
    if (Distance <= AcceptanceRadius)
    {
        ...
        if (ACharacter* Character = Cast<ACharacter>(Pawn))
        {
            if (UCharacterMovementComponent* MoveComp = Character->GetCharacterMovement())
            {
                ...
                // 계산된 공격자 배치 위치로 정확히 이동 (방어자로부터 50.0f 떨어진 위치)
...
FVector UBTTask_MoveAttackerToDefenderTile::CalculateAttackerPositionOnDefenderTile(AUnitAIController* AIController, AUnitCharacterBase* Defender) const
{
    ...
    float OffsetDistance = 50.0f; // 방어자로부터 떨어진 거리 (타일 크기에 따라 조정 가능)
