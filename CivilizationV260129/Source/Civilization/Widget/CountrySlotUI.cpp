...
void UCountrySlotUI::SetupForPlayer(int32 PlayerIndex, ASuperPlayerState* PlayerState)
{
	...
	// CountryImg 설정 (둥근 이미지 사용)
	if (CountryImg)
	{
		UTexture2D* Texture = PlayerState->GetCountryRoundImg();
		if (Texture)
		{
	...
	// CountryBtn 배경색을 국경선 색으로 설정
	if (CountryBtn)
	{
		FLinearColor BorderColor = PlayerState->BorderColor;
		CountryBtn->SetBackgroundColor(BorderColor);
	}
}
