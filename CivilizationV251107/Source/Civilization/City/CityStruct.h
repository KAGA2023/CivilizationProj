...
struct CIVILIZATION_API FCityData
{
    ...
    TArray<FName> BuiltBuildings;                                           // 건설된 건물들 (RowName으로 저장, 같은 타입도 여러 개 가능)
...
struct CIVILIZATION_API FCityCurrentStat
{
    ...
    EProductionType ProductionType = EProductionType::None;               // 생산 타입 (None/Building/Unit)

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Production")
    FName ProductionName = NAME_None;                                       // 생산 중인 건물/유닛 이름 (RowName)
