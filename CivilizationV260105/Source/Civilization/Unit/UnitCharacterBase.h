...
protected:
    ...
    // 피격 감지용 Body 콜리전 컴포넌트 (pre-Body 타입)
    ...
    // 무기 콜리전 컴포넌트 (pre-Weapon 타입, 시각화용)
    UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Combat Weapon")
    class UCapsuleComponent* WeaponCollisionComp;
...
public:
    ...
    class UCapsuleComponent* GetBodyCollisionComp() const { return BodyCollisionComp; }

    // 무기 메시 컴포넌트 접근
    UFUNCTION(BlueprintCallable, Category = "Combat Weapon")
    class UStaticMeshComponent* GetWeaponMeshComponent() const { return WeaponMeshComponent; }

    // 무기 콜리전 컴포넌트 접근
    UFUNCTION(BlueprintCallable, Category = "Combat Weapon")
    class UCapsuleComponent* GetWeaponCollisionComp() const { return WeaponCollisionComp; }

    // 피격 이벤트 델리게이트 (무기 콜리전이 이 Body 콜리전과 충돌 시 발생)
    DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnHitCollisionReceived);

    UPROPERTY(BlueprintAssignable, Category = "Combat Events")
    FOnHitCollisionReceived OnHitCollisionReceived;

protected:
    // 내부 함수들
    void LoadUnitDataTable();
    void SetupUnitMesh(const FUnitData& UnitData);
    void SetupUnitWeapon(const FUnitData& UnitData);
    void SetupUnitSounds(const FUnitData& UnitData);
    void SetupUnitUI(const FUnitData& UnitData);

    // 콜리전 이벤트 핸들러
    UFUNCTION()
    void OnBodyCollisionBeginOverlap(UPrimitiveComponent* OverlappedComponent,
                                     AActor* OtherActor,
