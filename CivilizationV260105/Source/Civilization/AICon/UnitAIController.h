// Fill out your copyright notice in the Description page of Project Settings.

#pragma once

#include "CoreMinimal.h"
#include "AIController.h"
#include "../Combat/UnitCombatStruct.h"
#include "UnitAIController.generated.h"

class UWorldComponent;
class UUnitManager;
class AUnitCharacterBase;

UCLASS()
class CIVILIZATION_API AUnitAIController : public AAIController
{
    GENERATED_BODY()

public:
    AUnitAIController();

protected:
    virtual void BeginPlay() override;
    virtual void OnPossess(APawn* InPawn) override;

    // Blackboard와 BehaviorTree 초기화
    void InitBlackboardAndBT();

private:
    // BehaviorTree와 Blackboard 데이터
    UPROPERTY()
    TObjectPtr<class UBehaviorTree> BTData;

    UPROPERTY()
    TObjectPtr<class UBlackboardData> BBData;

    // WorldComponent 참조
    UPROPERTY()
    TObjectPtr<UWorldComponent> WorldComponent = nullptr;

    // UnitManager 참조 (선택적)
    UPROPERTY()
    TObjectPtr<UUnitManager> UnitManager = nullptr;

    // 내부 이동 경로 관리 (Blackboard는 배열을 지원하지 않으므로 내부 변수로 관리)
    TArray<FVector2D> CurrentMovementPath;
    int32 CurrentPathIndex = 0;

    // 전투 관련 내부 변수 (구조체로 통합)
    UPROPERTY()
    FCombatVisualizationData CurrentCombatData;  // 전투 시각화 데이터

    bool bIsInCombat = false;  // 현재 전투 중인지 여부

    // Blackboard 키 이름 상수
    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyTargetHexPosition = TEXT("TargetHexPosition");

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyMovementComplete = TEXT("MovementComplete");

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyIsMoving = TEXT("IsMoving");

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyIsInCombat = TEXT("IsInCombat");

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyCombatComplete = TEXT("CombatComplete");

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyIsRangedAttack = TEXT("IsRangedAttack");

    // 전투 시각화 관련 Blackboard 키
    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyIsAttacker = TEXT("IsAttacker");  // 공격자/방어자 구분

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyDefenderRotateReady = TEXT("DefenderRotateReady");  // 방어자 회전 완료

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyAttackerMoveReady = TEXT("AttackerMoveReady");  // 공격자 이동 완료

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyAttackerAnimationStarted = TEXT("AttackerAnimationStarted");  // 공격자 공격 애니메이션 시작

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyAttackerAnimationFinished = TEXT("AttackerAnimationFinished");  // 공격자 공격 애니메이션 완료

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyDefenderHitFinished = TEXT("DefenderHitFinished");  // 방어자 피격 애니메이션 완료

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyCanCounterAttack = TEXT("CanCounterAttack");  // 반격 가능 여부

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyDefenderAnimationStarted = TEXT("DefenderAnimationStarted");  // 방어자 공격 애니메이션 시작

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyDefenderAnimationFinished = TEXT("DefenderAnimationFinished");  // 방어자 공격 애니메이션 완료

    UPROPERTY(EditDefaultsOnly, Category = "BBKeys")
    FName KeyAttackerHitFinished = TEXT("AttackerHitFinished");  // 공격자 피격 애니메이션 완료

public:
    // WorldComponent 설정
    UFUNCTION(BlueprintCallable, Category = "Setup")
    void SetWorldComponent(UWorldComponent* InWorldComponent);

    // WorldComponent 가져오기
    UFUNCTION(BlueprintCallable, Category = "Setup")
    UWorldComponent* GetWorldComponent() const { return WorldComponent; }

    // UnitManager 설정
    UFUNCTION(BlueprintCallable, Category = "Setup")
    void SetUnitManager(UUnitManager* InUnitManager);

    // 이동 명령 함수들
    UFUNCTION(BlueprintCallable, Category = "Unit Movement")
    void StartMovementAlongPath(const TArray<FVector2D>& Path);

    // 다음 헥스로 이동 (내부 함수, Task에서 호출)
    UFUNCTION(BlueprintCallable, Category = "Unit Movement")
    void MoveToNextHexInPath();

    // 이동 완료 처리
    UFUNCTION(BlueprintCallable, Category = "Unit Movement")
    void CompleteMovement();

    // 헥스 좌표 관련 함수들
    UFUNCTION(BlueprintCallable, Category = "Hex Position")
    FVector2D GetCurrentHexPosition() const;

    UFUNCTION(BlueprintCallable, Category = "Hex Position")
    FVector HexToWorld(FVector2D HexPosition) const;

    UFUNCTION(BlueprintCallable, Category = "Hex Position")
    FVector2D WorldToHex(FVector WorldPosition) const;

    // 현재 이동 경로 가져오기
    UFUNCTION(BlueprintCallable, Category = "Unit Movement")
    TArray<FVector2D> GetCurrentMovementPath() const { return CurrentMovementPath; }

    // 현재 경로 인덱스 가져오기
    UFUNCTION(BlueprintCallable, Category = "Unit Movement")
    int32 GetCurrentPathIndex() const { return CurrentPathIndex; }

    // 이동 중인지 확인
    UFUNCTION(BlueprintCallable, Category = "Unit Movement")
    bool IsMoving() const { return CurrentMovementPath.Num() > 0 && CurrentPathIndex < CurrentMovementPath.Num(); }

    // 전투 명령 함수들
    UFUNCTION(BlueprintCallable, Category = "Unit Combat")
    void StartCombatVisualization(AUnitCharacterBase* Attacker, AUnitCharacterBase* Defender, const FCombatResult& CombatResult);

    // 전투 완료 처리
    UFUNCTION(BlueprintCallable, Category = "Unit Combat")
    void CompleteCombatVisualization();

    // 전투 중인지 확인
    UFUNCTION(BlueprintCallable, Category = "Unit Combat")
    bool IsInCombat() const { return bIsInCombat; }

    // 현재 전투 데이터 가져오기 (구조체 멤버 접근)
    UFUNCTION(BlueprintCallable, Category = "Unit Combat")
    AUnitCharacterBase* GetCombatAttacker() const { return CurrentCombatData.Attacker; }

    UFUNCTION(BlueprintCallable, Category = "Unit Combat")
    AUnitCharacterBase* GetCombatDefender() const { return CurrentCombatData.Defender; }

    UFUNCTION(BlueprintCallable, Category = "Unit Combat")
    FCombatResult GetCurrentCombatResult() const { return CurrentCombatData.CombatResult; }

    // 공격자 원래 위치 가져오기
    UFUNCTION(BlueprintCallable, Category = "Unit Combat")
    FVector2D GetAttackerOriginalHexPosition() const { return CurrentCombatData.AttackerOriginalHexPosition; }

    // 전투 사거리 정보 접근 함수
    UFUNCTION(BlueprintCallable, Category = "Unit Combat")
    int32 GetCombatRange() const { return CurrentCombatData.CombatRange; }

    UFUNCTION(BlueprintCallable, Category = "Unit Combat")
    bool IsRangedAttack() const { return CurrentCombatData.bIsRangedAttack; }

    // 전투 시각화 데이터 전체 가져오기
    UFUNCTION(BlueprintCallable, Category = "Unit Combat")
    FCombatVisualizationData GetCombatVisualizationData() const { return CurrentCombatData; }

    // ========== 콜리전 이벤트 핸들러 ==========
    // 피격 콜리전 수신 (공격/반격/원거리 모두 처리)
    UFUNCTION()
    void OnHitCollisionReceived();
};

