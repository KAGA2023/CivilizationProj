...
void UWorldSettingMenuUI::NativeConstruct()
{
	...
	if (TemperateRatioSlider)
	{
		...
		if (TemperateRatioTxt)
		{
			...
			float RoundedValue = FMath::RoundToFloat(InitialValue * 10.0f) / 10.0f;
			int32 PercentValue = FMath::RoundToInt(RoundedValue * 100.0f);
			TemperateRatioTxt->SetText(FText::FromString(FString::Printf(TEXT("%d%%"), (100 - PercentValue))));
		}

		// 사막과 툰드라 비율 초기 텍스트 값 설정
		if (DesertAndTundraRatioTxt)
		{
			float InitialValue = TemperateRatioSlider->GetValue();
			float RoundedValue = FMath::RoundToFloat(InitialValue * 10.0f) / 10.0f;
			int32 PercentValue = FMath::RoundToInt(RoundedValue * 100.0f);
			DesertAndTundraRatioTxt->SetText(FText::FromString(FString::Printf(TEXT("%d%%"), PercentValue)));
		}
	}
	...
	if (PlainRatioSlider)
	{
		...
		if (PlainRatioTxt)
		{
			...
			float RoundedValue = FMath::RoundToFloat(InitialValue * 10.0f) / 10.0f;
			int32 PercentValue = FMath::RoundToInt(RoundedValue * 100.0f);
			PlainRatioTxt->SetText(FText::FromString(FString::Printf(TEXT("%d%%"), (100 - PercentValue))));
		}

		// 언덕과 산 비율 초기 텍스트 값 설정
		if (HillAndMountainRatioTxt)
		{
			float InitialValue = PlainRatioSlider->GetValue();
			float RoundedValue = FMath::RoundToFloat(InitialValue * 10.0f) / 10.0f;
			int32 PercentValue = FMath::RoundToInt(RoundedValue * 100.0f);
			HillAndMountainRatioTxt->SetText(FText::FromString(FString::Printf(TEXT("%d%%"), PercentValue)));
		}
	}
...
void UWorldSettingMenuUI::OnStartButtonClicked()
{
	...
	if (TemperateRatioSlider && DesertRatioSlider)
	{
		...
		// TemperateRatioSlider의 (1.0 - value)가 온대 비율
		NewWorldConfig.TemperatePercentage = 1.0f - TemperateSliderValue;

		// 나머지 (TemperateSliderValue)를 사막과 툰드라가 나눔
		// DesertSliderValue의 (1.0 - value)가 사막 비율
		NewWorldConfig.DesertPercentage = TemperateSliderValue * (1.0f - DesertSliderValue);

		// DesertSliderValue가 툰드라 비율
		NewWorldConfig.TundraPercentage = TemperateSliderValue * DesertSliderValue;
	}
	...
	if (PlainRatioSlider && HillRatioSlider)
	{
		...
		// PlainRatioSlider의 (1.0 - value)가 평지 비율
		NewWorldConfig.PlainsPercentage = 1.0f - PlainSliderValue;

		// 나머지 (PlainSliderValue)를 언덕과 산이 나눔
		// HillSliderValue의 (1.0 - value)가 언덕 비율
		NewWorldConfig.HillsPercentage = PlainSliderValue * (1.0f - HillSliderValue);

		// HillSliderValue가 산 비율
		NewWorldConfig.MountainPercentage = PlainSliderValue * HillSliderValue;
	}
...
void UWorldSettingMenuUI::InitializeSlotWithNoSelect(UCountrySelectSlotUI* TargetSlot)
{
	...
	// 이미지 설정
	if (!CountryData->CountrySmallImg.IsNull())
	{
		UTexture2D* CountryImage = CountryData->CountrySmallImg.LoadSynchronous();
		if (CountryImage)
		{
...
void UWorldSettingMenuUI::InitializeCountrySelectMiniSlots()
{
	...
	if (!NoSelectData->CountrySmallImg.IsNull())
	{
		NoSelectImage = NoSelectData->CountrySmallImg.LoadSynchronous();
	}
	...
	for (int32 i = 0; i < TotalSlots && i < CountryRowNames.Num(); i++)
	{
		...
		if (CountryData && !CountryData->CountrySmallImg.IsNull())
		{
			CountryImages[i] = CountryData->CountrySmallImg.LoadSynchronous();
		}
		else
		{
...
void UWorldSettingMenuUI::InitializeHoveredCountryUI()
{
	...
	if (HoveredCountryImg)
	{
		if (!NoSelectData->CountrySmallImg.IsNull())
		{
			UTexture2D* NoSelectImage = NoSelectData->CountrySmallImg.LoadSynchronous();
			if (NoSelectImage)
			{
