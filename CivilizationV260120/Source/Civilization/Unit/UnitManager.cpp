    ...
    if (USmallUnitUI* SmallUI = NewUnit->GetSmallUnitUI())
    {
        ...
        if (PlayerIndex >= 0)
        {
            if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(World->GetGameInstance()))
            {
                ...
                {
                    if (!PlayerState->CountrySmallImg.IsNull())
                    {
                        UTexture2D* CountryTexture = PlayerState->CountrySmallImg.LoadSynchronous();
...
void UUnitManager::SetSelectedUnit(AUnitCharacterBase* Unit)
{
    ...
    {
        CurrentSelectedUnit->SetUnitSelected(true);
        CurrentSelectedUnit->SetSelectionColor(FLinearColor::White);
    }
}
