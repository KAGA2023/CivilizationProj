...
void UAIPlayerManager::ProcessFacilityState(int32 PlayerIndex)
{
	...
	if (AIPlayer->TargetFacilityTile.X >= 0 && AIPlayer->TargetFacilityTile.Y >= 0)
	{
	...
	for (UWorldTile* Tile : OwnedTiles)
	{
		...
		ValidTiles.Add(TileCoord);
	}

	// 유효한 타일이 없으면 다음 상태로 전환
	if (ValidTiles.Num() == 0)
	{
		TransitionToNextState(PlayerIndex);
		return;
	...
	FVector2D SelectedTile = ValidTiles[RandomIndex];
