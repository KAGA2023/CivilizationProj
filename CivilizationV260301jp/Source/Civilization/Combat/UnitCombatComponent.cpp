...
FCombatResult UUnitCombatComponent::ExecuteCombat(AUnitCharacterBase* Attacker, AUnitCharacterBase* Defender, int32 HexDistance, FVector2D AttackerHex, FVector2D DefenderHex)
{
    ...
    // 3) 층수 차이 1 이하
    if (HexDistance == 1 && AttackerRange == 1 && DefenderRange == 1)
    {
    ...
    // ========== 데미지 적용 ==========
    // 1) 먼저 공격자의 데미지를 방어자에게 적용
    DefenderStatus->TakeDamage(FinalAttackDamage);

    // 2) 방어자가 살아있고, 반격 데미지가 있을 때만 반격 적용
    //    - 조건: 방어자가 사망하면 반격 없음
    //    - 조건: 반격 데미지로 공격자를 죽일 수 없음 (최소 체력 1 보장)
    if (FinalCounterDamage > 0 && !DefenderStatus->IsDead())
    {
        // 공격자의 현재 체력 확인
        ...
        int32 ActualCounterDamage = FMath::Min(FinalCounterDamage, MaxAllowedCounterDamage);
        
        if (ActualCounterDamage > 0)
        {
            AttackerStatus->TakeDamage(ActualCounterDamage);
        }
        
        // 결과에 실제 적용된 반격 데미지 기록
        FinalCounterDamage = ActualCounterDamage;
    }
...
FCombatResult UUnitCombatComponent::ExecuteCombatAgainstCity(AUnitCharacterBase* Attacker, UCityComponent* CityComponent, int32 HexDistance, FVector2D AttackerHex, FVector2D CityHex)
{
    ...
    // 도시 스모그 파티클 visible 갱신 (75% / 50% / 25% 기준)
    if (UWorld* World = Attacker ? Attacker->GetWorld() : nullptr)
    {
        if (AWorldSpawner* WorldSpawner = Cast<AWorldSpawner>(UGameplayStatics::GetActorOfClass(World, AWorldSpawner::StaticClass())))
        {
            const FVector2D RoundedCityHex(FMath::RoundToInt(CityHex.X), FMath::RoundToInt(CityHex.Y));
            if (ACityActor* CityActor = WorldSpawner->GetCityActorAtHex(RoundedCityHex))
            {
                CityActor->UpdateCitySmogVisibility(CityComponent->GetCurrentHealth(), CityComponent->GetMaxHealth());
                if (ASuperPlayerState* OwnerPS = Cast<ASuperPlayerState>(CityComponent->GetOwner()))
                {
                    CityActor->UpdateSmallCityUI(OwnerPS->GetCountryName(), OwnerPS->GetCountryLargeImg(), CityComponent->GetCurrentHealth(), CityComponent->GetMaxHealth());
                }
            }
        }
    }
...
FText UUnitCombatComponent::GetTerrainBonusText(FVector2D MyHex, FVector2D EnemyHex, int32 UnitRange) const
{
    ...
    if (UnitRange <= 1)
    {
        ...
        if (HeightBonus > 0)
        {
            ResultText += FString::Printf(TEXT("高地帯ボーナス\n+%d"), HeightBonus);
        }
        ...
        if (ForestBonus > 0)
        {
            ResultText += FString::Printf(TEXT("森ボーナス\n+%d"), ForestBonus);
        }
    }
    ...
    if (UnitRange > 1)
    {
        ...
        if (RangeBonus > 0)
        {
            ...
            ResultText += FString::Printf(TEXT("高地帯射程ボーナス\n+%d"), RangeBonus);
        }
    }
