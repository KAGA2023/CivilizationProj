...
bool UUnitManager::CanPlayerEnterTile(FVector2D HexPosition, int32 MoverPlayerIndex) const
{
    if (!WorldComponent)
    {
        return false;
    ...
    int32 OwnerID = Tile->GetOwnerPlayerID();
    // 본인 소유 → 진입 가능
    if (OwnerID == MoverPlayerIndex)
    {
        return true;
    }
    // 타국 소유 → 전쟁 또는 동맹일 때만 진입 가능
    UWorld* World = GetWorld();
    if (!World)
    {
        return false;
    ...
    EDiplomacyStatusType Status = DiplomacyManager->GetStatus(MoverPlayerIndex, OwnerID);
    return (Status == EDiplomacyStatusType::War || Status == EDiplomacyStatusType::Alliance);
}

bool UUnitManager::FindNearestValidTileForEviction(FVector2D FromHex, AUnitCharacterBase* Unit, FVector2D& OutHex) const
{
    OutHex = FVector2D::ZeroVector;
    if (!WorldComponent || !Unit)
    {
        return false;
    }
    const FVector2D RoundedFrom(FMath::RoundToInt(FromHex.X), FMath::RoundToInt(FromHex.Y));
    TQueue<FVector2D> Queue;
    TSet<FVector2D> Visited;
    Queue.Enqueue(RoundedFrom);
    Visited.Add(RoundedFrom);
    while (!Queue.IsEmpty())
    {
        FVector2D Current;
        ...
        {
            const FVector2D RoundedNeighbor(FMath::RoundToInt(Neighbor.X), FMath::RoundToInt(Neighbor.Y));
            if (Visited.Contains(RoundedNeighbor))
            {
                continue;
            }
            if (!WorldComponent->IsValidHexPosition(RoundedNeighbor))
            {
                continue;
            }
            Visited.Add(RoundedNeighbor);
            if (CanPlaceUnitAtHex(RoundedNeighbor, Unit))
            {
                OutHex = RoundedNeighbor;
                return true;
            }
            Queue.Enqueue(RoundedNeighbor);
        }
    }
    return false;
}

bool UUnitManager::EvictUnitToNearestValidTile(AUnitCharacterBase* Unit)
{
    if (!Unit || !WorldComponent)
    {
        return false;
    }
    FVector2D CurrentHex = GetHexPositionForUnit(Unit);
    if (CurrentHex == FVector2D::ZeroVector)
    {
        return false;
    }
    const FVector2D RoundedCurrent(FMath::RoundToInt(CurrentHex.X), FMath::RoundToInt(CurrentHex.Y));
    FVector2D DestHex;
    if (!FindNearestValidTileForEviction(RoundedCurrent, Unit, DestHex))
    {
        return false;
    }
    RemoveUnitFromHex(RoundedCurrent);
    if (!SetUnitAtHex(DestHex, Unit))
    {
        SetUnitAtHex(RoundedCurrent, Unit);
        return false;
    }
    FVector WorldPos;
    if (UWorldTile* Tile = WorldComponent->GetTileAtHex(DestHex))
    {
        WorldPos = Tile->GetWorldPosition();
        switch (Tile->GetLandType())
        {
            case ELandType::Plains:    WorldPos.Z = 73.0f;  break;
            case ELandType::Hills:     WorldPos.Z = 146.0f; break;
            case ELandType::Mountains: WorldPos.Z = 219.0f; break;
            default:                   WorldPos.Z = 73.0f; break;
        }
    }
    else
    {
        WorldPos = WorldComponent->HexToWorld(DestHex);
        WorldPos.Z = 73.0f;
    }
...
void UUnitManager::EvictUnitsOnForeignTilesBetweenPlayers(int32 PlayerA, int32 PlayerB)
{
    if (!WorldComponent)
    {
        return;
    ...
    for (AUnitCharacterBase* Unit : SpawnedUnits)
    {
        ...
        int32 UnitPlayer = Unit->GetPlayerIndex();
        FVector2D UnitHex = GetHexPositionForUnit(Unit);
        if (UnitHex == FVector2D::ZeroVector)
        {
            continue;
        }
        UWorldTile* Tile = WorldComponent->GetTileAtHex(UnitHex);
        if (!Tile || !Tile->IsOwned())
        {
            continue;
        }
        int32 TileOwner = Tile->GetOwnerPlayerID();
        bool bShouldEvict = false;
        if (UnitPlayer == PlayerA && TileOwner == PlayerB)
        {
            bShouldEvict = true;
        }
        else if (UnitPlayer == PlayerB && TileOwner == PlayerA)
        {
            bShouldEvict = true;
        }
        if (bShouldEvict)
        {
            EvictUnitToNearestValidTile(Unit);
        }
    }
}
...
bool UUnitManager::CanPlaceUnitAtHex(FVector2D HexPosition, AUnitCharacterBase* MovingUnit) const
{
    ...
    // 국경선: 이동 유닛이 있으면 해당 플레이어가 진입 가능한 타일인지 확인
    if (MovingUnit)
    {
        if (!CanPlayerEnterTile(HexPosition, MovingUnit->GetPlayerIndex()))
        {
            return false;
        }
    }
...
void UUnitManager::MoveUnitFromFirstToSecondSelection()
{
    ...
    // A* 경로 찾기로 최적 경로 계산 (국경선 적용)
    TArray<FVector2D> Path = FindPath(FirstHexPos, SecondHexPos, MoverPlayerIndex);
    ...
    // 이동력 제한이 있는 경로 찾기
    TArray<FVector2D> LimitedPath = FindPathWithMovementCost(FirstHexPos, SecondHexPos, MaxMovementCost, MoverPlayerIndex);
...
// 경로 찾기 및 이동 시스템 구현
TArray<FVector2D> UUnitManager::FindPath(FVector2D StartHex, FVector2D EndHex, int32 MoverPlayerIndex) const
{
    ...
    // 목표점에 도달할 수 없는지 확인 (국경선 포함)
    if (!CanMoveToHex(EndHex, MoverPlayerIndex))
    {
        return Path; // 빈 경로 반환
    }
    ...
    while (OpenSet.Num() > 0)
    {
        ...
        {
            // 유효하지 않은 위치이거나 층수·국경선 이동이 불가능한 곳은 건너뛰기
            if (!WorldComponent->IsValidHexPosition(NeighborHex) || !CanMoveBetweenHexes(CurrentHex, NeighborHex, MoverPlayerIndex))
            {
                continue;
            }
            ...
            int32 MovementCost = GetMovementCostBetweenHexes(CurrentHex, NeighborHex, MoverPlayerIndex);
...
TArray<FVector2D> UUnitManager::FindPathWithMovementCost(FVector2D StartHex, FVector2D EndHex, int32 MaxMovementCost, int32 MoverPlayerIndex) const
{
    // A* 알고리즘으로 경로를 찾은 후, 최대 이동 비용을 초과하는지 확인
    TArray<FVector2D> Path = FindPath(StartHex, EndHex, MoverPlayerIndex);
    ...
    {
        TotalMovementCost += GetMovementCostBetweenHexes(Path[i], Path[i + 1], MoverPlayerIndex);
...
bool UUnitManager::CanMoveToHex(FVector2D HexPosition, int32 MoverPlayerIndex) const
{
    if (!WorldComponent)
    {
        return false;
    ...
    // 국경선: 본인/무주 또는 전쟁·동맹국 타일만 이동 가능
    return CanPlayerEnterTile(HexPosition, MoverPlayerIndex);
}
...
int32 UUnitManager::GetMovementCostBetweenHexes(FVector2D FromHex, FVector2D ToHex, int32 MoverPlayerIndex) const
{
    // 층수·국경선 이동 가능성 먼저 체크
    if (!CanMoveBetweenHexes(FromHex, ToHex, MoverPlayerIndex))
    {
        return INT32_MAX;
    }
    
    // 층수를 고려한 이동비용 계산
    return GetMovementCostBetweenHexesWithFloor(FromHex, ToHex, MoverPlayerIndex);
}
...
bool UUnitManager::CanMoveBetweenHexes(FVector2D FromHex, FVector2D ToHex, int32 MoverPlayerIndex) const
{
    if (!WorldComponent)
    {
        return false;
    ...
    // 국경선: 도착 타일 진입 가능 여부
    if (!CanPlayerEnterTile(ToHex, MoverPlayerIndex))
    {
        return false;
    }
...
bool UUnitManager::IsUnitTrapped(AUnitCharacterBase* Unit) const
{
    if (!Unit || !WorldComponent)
    {
        return false;
    ...
    FVector2D UnitHex = GetHexPositionForUnit(Unit);
    const FVector2D RoundedHex(FMath::RoundToInt(UnitHex.X), FMath::RoundToInt(UnitHex.Y));

    if (!WorldComponent->IsValidHexPosition(RoundedHex))
    {
        return false;
    }

    const TArray<FVector2D> Neighbors = WorldComponent->GetHexNeighbors(RoundedHex);
    for (const FVector2D& Neighbor : Neighbors)
    {
        const FVector2D RoundedNeighbor(FMath::RoundToInt(Neighbor.X), FMath::RoundToInt(Neighbor.Y));
        if (!WorldComponent->IsValidHexPosition(RoundedNeighbor))
        ...
        if (CanMoveBetweenHexes(RoundedHex, RoundedNeighbor, Unit->GetPlayerIndex()))
        {
            return false; // 한 칸이라도 이동 가능하면 갇히지 않음
        }
    }

    return true; // 모든 인접 타일 이동 불가 → 갇힌 유닛
}

int32 UUnitManager::GetMovementCostBetweenHexesWithFloor(FVector2D FromHex, FVector2D ToHex, int32 MoverPlayerIndex) const
{
    if (!WorldComponent)
    {
        return INT32_MAX;
    ...
    // 먼저 이동 가능한지 확인 (층수·국경선 검증 포함)
    if (!CanMoveBetweenHexes(FromHex, ToHex, MoverPlayerIndex))
    {
        return INT32_MAX; // 이동 불가 (층수 차이 2 이상, 바다, 또는 국경선 미통과)
    }
...
void UUnitManager::RequestBuilderBuildFacility(FVector2D Hex, FName FacilityRowName)
{
    ...
    if (UnitAtTile && IsBuilderUnit(UnitAtTile))
    {
        ...
        {
            // 건설자만: 몽타주(건설 모션) 시작 직후 스팀 이펙트 스폰
            FacilityManager->SpawnBuildSteamAtTile(Hex, WorldComp);
    ...
    // 건설자가 아닌 경우 또는 몽타주 없이 실행되는 경우: 스팀 없이 건설만 수행
    FacilityManager->BuildFacility(FacilityRowName, Hex, WorldComp);
}
...
void UUnitManager::StartVisualMovement(AUnitCharacterBase* Unit, const TArray<FVector2D>& Path)
{
    ...
    if (UUnitStatusComponent* StatusComp = Unit->GetUnitStatusComponent())
    {
        ...
        int32 MoverPlayerIndex = Unit->GetPlayerIndex();
        for (int32 i = 0; i < Path.Num() - 1; i++)
        {
            int32 TileCost = GetMovementCostBetweenHexes(Path[i], Path[i + 1], MoverPlayerIndex);
            TotalCost += TileCost;
        }
...
{
    if (!Unit || !WorldComponent || Path.Num() <= 1)
    {
        return;
    }

    // 타일 좌표를 정수로 보정 (소수점 방지)
    FVector2D StartHex(
        FMath::RoundToInt(Path[0].X),
        FMath::RoundToInt(Path[0].Y)
    );
    FVector2D DestinationHex(
        FMath::RoundToInt(Path[Path.Num() - 1].X),
        FMath::RoundToInt(Path[Path.Num() - 1].Y)
    );
    ...
    // 이동 비용 소비 (StartVisualMovement와 동일)
    if (UUnitStatusComponent* StatusComp = Unit->GetUnitStatusComponent())
    {
        int32 TotalCost = 0;
        int32 MoverPlayerIndex = Unit->GetPlayerIndex();
    ...
    RemoveUnitFromHex(StartHex);
    SetUnitAtHex(DestinationHex, Unit);

    // 액터를 도착 타일 월드 좌표로 텔레포트 (평지 73 / 언덕 146 / 산 219 Z 높이)
    FVector WorldPos;
    if (UWorldTile* Tile = WorldComponent->GetTileAtHex(DestinationHex))
    {
        WorldPos = Tile->GetWorldPosition();
        switch (Tile->GetLandType())
        {
    ...
    {
        WorldPos = WorldComponent->HexToWorld(DestinationHex);
        WorldPos.Z = 73.0f;
    }
    Unit->SetActorLocation(WorldPos);

    OnUnitMovementComplete(Unit, DestinationHex);
}

void UUnitManager::CompleteMovement()
{
    // 이동 완료 시점에는 선택 해제·이동 선택 정리 하지 않음 (다른 유닛이 이미 선택된 경우 유지)
    // 전사 선택 해제·이동 선택 해제는 전사가 이동을 시작할 때 이미 수행됨
}
...
void UUnitManager::SetSelectedUnit(AUnitCharacterBase* Unit)
{
    ...
    // 선택 유닛 변경 델리게이트 브로드캐스트 (UnitInfoUI 등에서 사용)
    OnSelectedUnitChanged.Broadcast(CurrentSelectedUnit);

    // 선택된 유닛의 SelectSound 1회 재생 (DT_UnitData의 FUnitData.SelectSound)
    if (CurrentSelectedUnit)
    {
        if (const FUnitData* UnitData = CurrentSelectedUnit->GetUnitData())
        {
            if (UnitData->SelectSound)
            {
                UGameplayStatics::PlaySound2D(this, UnitData->SelectSound);
            }
        }
    }
}
...
void UUnitManager::ClearSelectedUnit()
{
    ...
    // 선택 해제 시에도 항상 브로드캐스트 (UnitInfoUI 숨김 동기화)
    OnSelectedUnitChanged.Broadcast(nullptr);
}
...
TArray<FVector2D> UUnitManager::CalculateMovementRange(AUnitCharacterBase* Unit) const
{
    ...
    while (Queue.Num() > 0)
    {
        ...
        {
            // A*와 동일: 유효하지 않은 좌표는 건너뛰기
            if (!WorldComponent->IsValidHexPosition(Neighbor))
            {
                continue;
            }
            // 이동 가능한지 확인 (국경선 포함, 도시 타일 통행불가 포함)
            if (!CanMoveToHex(Neighbor, Unit->GetPlayerIndex()))
            {
                continue; // 이동 불가능한 타일은 건너뛰기
            }
            
            // 다른 유닛이 있는 타일은 지나갈 수 없음 (A*와 동일하게 벽으로 취급)
            if (AUnitCharacterBase* UnitAtNeighbor = GetUnitAtHex(Neighbor))
            {
                if (UnitAtNeighbor != Unit)
                {
                    continue; // 다른 유닛이 있으면 확장하지 않음
                }
            }
            
            // 이동 비용 계산 (층수 차이 포함)
            int32 MoveCost = GetMovementCostBetweenHexesWithFloor(Current, Neighbor, Unit->GetPlayerIndex());
