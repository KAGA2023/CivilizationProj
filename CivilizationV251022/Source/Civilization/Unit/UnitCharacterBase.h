...
protected:
    ...
    // 데이터 테이블 참조들
    UPROPERTY()
    UDataTable* UnitSpawnTable = nullptr;
...
public:
    ...
    void InitializeUnit(const FName& UnitID);

    UFUNCTION(BlueprintCallable, Category = "Unit Initialization")
    void InitializeUnitFromData(const FUnitSpawnData& SpawnData, const FUnitData& UnitData, const FUnitBaseStat& UnitStat);
