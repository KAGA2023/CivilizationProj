...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "Building Production")
    void SetProductionProgress(int32 Progress); // 생산 진행도 직접 설정 (세이브/로드용)
    ...
    UFUNCTION(BlueprintCallable, Category = "Unit Production")
    void SetFoodProgress(int32 Progress); // 식량 진행도 직접 설정 (세이브/로드용)
    ...
    void StopCurrentProduction(); // 현재 생산 중인 건물/유닛 생산 중단 및 초기화
