...
AFacilityActor* UFacilityManager::SpawnFacilityAtHex(FVector2D TileCoordinate, FName FacilityRowName, UWorldComponent* WorldComponent)
{
	...
	// 자원이 있으면 해당 자원 데이터의 FacilityMesh와 PillagedMesh 사용
	if (StrategicResource != EStrategicResource::None)
	{
		// 전략 자원 데이터 테이블에서 가져오기
		UDataTable* StrategicResourceDataTable = WorldComponent->GetStrategicResourceDataTable();
		if (StrategicResourceDataTable)
		{
			// Enum을 FName으로 변환 (WorldComponent.cpp의 방식을 따라 "::" 이후만 가져오기)
			...
			FStrategicResourceData* ResourceData = StrategicResourceDataTable->FindRow<FStrategicResourceData>(
				ResourceRowName, TEXT("SpawnFacilityAtHex"));
			
			if (ResourceData)
			{
				if (ResourceData->FacilityMesh)
				{
					MeshToUse = ResourceData->FacilityMesh;
				}
				if (ResourceData->PillagedMesh)
				{
					PillagedMeshToUse = ResourceData->PillagedMesh;
				}
			}
		}
	}
	else if (LuxuryResource != ELuxuryResource::None)
	{
		...
		if (LuxuryResourceDataTable)
		{
			...
			FName ResourceRowName = FName(*EnumString.RightChop(EnumString.Find(TEXT("::")) + 2));
			FLuxuryResourceData* ResourceData = LuxuryResourceDataTable->FindRow<FLuxuryResourceData>(
				ResourceRowName, TEXT("SpawnFacilityAtHex"));
	...
	else if (BonusResource != EBonusResource::None)
	{
		...
		if (BonusResourceDataTable)
		{
			...
			FName ResourceRowName = FName(*EnumString.RightChop(EnumString.Find(TEXT("::")) + 2));
			FBonusResourceData* ResourceData = BonusResourceDataTable->FindRow<FBonusResourceData>(
				ResourceRowName, TEXT("SpawnFacilityAtHex"));
	...
	// 자원이 없거나 자원의 FacilityMesh가 None이면 FacilityData의 메시 사용
	if (!MeshToUse)
	{
