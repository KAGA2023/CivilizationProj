...
struct CIVILIZATION_API FBonusResourceData : public FTableRowBase
{
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Bonus Resource")
    TSoftObjectPtr<UTexture2D> ResourceIcon; // 자원 아이콘 이미지
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Bonus Resource")
    UStaticMesh* FacilityMesh = nullptr; // 시설 메시 (자원이 있는 타일에 시설 건설 시 사용)

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Bonus Resource")
    UStaticMesh* PillagedMesh = nullptr; // 약탈 상태 메시 (자원이 있는 타일에 시설 건설 시 사용)

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Bonus Resource | Forest+Resource")
    UStaticMesh* ForestResourceMesh = nullptr; // 숲+자원 타일용 합본 메시 (시설 없을 때)

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Bonus Resource | Forest+Resource")
    UStaticMesh* ForestFacilityMesh = nullptr; // 숲+자원 타일에 시설 건설 시 메시

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Bonus Resource | Forest+Resource")
    UStaticMesh* ForestFacilityPillagedMesh = nullptr; // 숲+자원 타일 시설 약탈 메시
...
struct CIVILIZATION_API FWorldConfig
{
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "World Settings")
    int32 PlayerCount = 4; // 총 플레이어 수 (플레이어 1명 + AI N명)
