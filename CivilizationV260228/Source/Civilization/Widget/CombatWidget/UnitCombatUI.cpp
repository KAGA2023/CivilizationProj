...
void UUnitCombatUI::SetupForCombat(AUnitCharacterBase* Attacker, AUnitCharacterBase* Defender, FVector2D AttackerHex, FVector2D DefenderHex)
{
	...
	if (EnemyUnitNameTxt)
	{
		...
		FText EnemyNameWithMark = FText::FromString(DefenderBaseStat.UnitName.ToString() + TEXT("(敵)"));
		EnemyUnitNameTxt->SetText(EnemyNameWithMark);
	}
	...
	{
		SetCombatResultImage(TEXT("勝利"), true, false, false);
	}
	// 방어자와 공격자가 모두 죽으면 전멸
	else if (bCanKillDefender && !bAttackerSurvives)
	{
		SetCombatResultImage(TEXT("全滅"), false, true, false);
	}
	// 방어자를 죽일 수 없고, 공격자가 죽는다면 패배
	else if (!bCanKillDefender && !bAttackerSurvives)
	{
		SetCombatResultImage(TEXT("敗北"), false, false, true);
	}
	...
	else if (!bCanKillDefender && bAttackerSurvives)
	{
		...
		{
			SetCombatResultImage(TEXT("有利"), true, false, false);
		}
		else if (DamageDifference <= -10)
		{
			SetCombatResultImage(TEXT("不利"), false, false, true);
		}
		else
		{
			SetCombatResultImage(TEXT("互角"), false, true, false);
		}
	}
	else
	{
...
void UUnitCombatUI::SetupForCombatAgainstCity(AUnitCharacterBase* Attacker, UCityComponent* CityComponent, FVector2D AttackerHex, FVector2D CityHex)
{
	...
	if (EnemyUnitNameTxt)
	{
		EnemyUnitNameTxt->SetText(FText::FromString(TEXT("敵 都市(敵)")));
	}
