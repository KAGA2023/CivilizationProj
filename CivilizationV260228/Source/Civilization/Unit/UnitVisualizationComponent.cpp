...
{
    UE_LOG(LogTemp, Log, TEXT("OnCombatNotify_Hit"));
    // 도시 공격: Notify_Combat 타이밍에 Destroy 사운드 재생
...
void UUnitVisualizationComponent::OnCombatNotify_HitOnly()
{
    ...
    // Notify_Hit: Hit 몽타주만 재생 (Death 없음). 연타 연출용. 근접과 동일하게 원거리에서도 방어자 HitSound + Hit 몽타주 재생.
    switch (CombatState)
    {
        case ECombatVisualizationState::AttackerAttack:
        ...
        case ECombatVisualizationState::AttackerAttack_Ranged:
            ...
            if (CombatDefender.IsValid() && CurrentCombatResult.bDefenderAlive)
            {
                if (const FUnitData* DefenderData = CombatDefender.Get()->GetUnitData())
                {
                    if (DefenderData->HitSound)
                    {
                        UGameplayStatics::PlaySound2D(this, DefenderData->HitSound);
            ...
            PlayDefenderHitMontageOnly_Ranged();
            break;
        case ECombatVisualizationState::DefenderCounter:
            if (CombatAttacker.IsValid())
            {
...
void UUnitVisualizationComponent::SpawnRangedProjectile()
{
    ...
    FVector EndLoc = Defender->GetMesh()
