...
void ULoadMenuUI::OnLoadButtonClicked()
{
	...
	if (SelectedSlotIndex <= 0)
	{
		if (WarningTxt)
		{
			WarningTxt->SetText(FText::FromString(TEXT("読み込むスロットが選択されていません！")));
	...
	if (UWorld* World = GetWorld())
	{
		if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(World->GetGameInstance()))
		{
			...
			else
			{
				...
				if (WarningTxt)
				{
					WarningTxt->SetText(FText::FromString(TEXT("このスロットには保存されたデータがありません！")));
					WarningTxt->SetVisibility(ESlateVisibility::Visible);
				}
			}
		}
...
void ULoadMenuUI::UpdateSlotInfo(int32 SlotIndex)
{
	...
	if (!SaveGameObject)
	{
		SlotExplainTxt->SetText(FText::FromString(TEXT("ロード失敗")));
		return;
	}
