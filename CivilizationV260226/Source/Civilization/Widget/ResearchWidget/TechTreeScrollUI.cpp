...
FString UTechTreeScrollUI::BuildUnlockInfoString(const FTechData& TechData) const
{
	...
	// 시설: DT_FacilityData에서 RowName으로 조회 → FacilityName
	UDataTable* FacilityTable = Cast<UDataTable>(FSoftObjectPath(TEXT("/Game/Civilization/Data/DT_FacilityData.DT_FacilityData")).TryLoad());
	for (const FName& RowName : TechData.UnlockFacilities)
	{
		FString DisplayName = RowName.ToString();
		if (FacilityTable)
		{
			if (FFacilityData* Row = FacilityTable->FindRow<FFacilityData>(RowName, TEXT("BuildUnlockInfoString")))
			{
				if (!Row->FacilityName.IsEmpty())
				{
					DisplayName = Row->FacilityName;
				}
			}
		}
		Lines.Add(FString::Printf(TEXT("施設 追加:\n +%s"), *DisplayName));
	}

	// 건물: DT_BuildingData에서 RowName으로 조회 → BuildingName
	UDataTable* BuildingTable = Cast<UDataTable>(FSoftObjectPath(TEXT("/Game/Civilization/Data/DT_BuildingData.DT_BuildingData")).TryLoad());
	for (const FName& RowName : TechData.UnlockBuildings)
	{
		FString DisplayName = RowName.ToString();
		if (BuildingTable)
		{
			if (FBuildingData* Row = BuildingTable->FindRow<FBuildingData>(RowName, TEXT("BuildUnlockInfoString")))
			{
				if (!Row->BuildingName.IsEmpty())
				{
					DisplayName = Row->BuildingName;
				}
			}
		}
		Lines.Add(FString::Printf(TEXT("建造物 追加:\n +%s"), *DisplayName));
	}

	// 유닛: DT_UnitBaseStat에서 RowName으로 조회 → UnitName
	UDataTable* UnitTable = Cast<UDataTable>(FSoftObjectPath(TEXT("/Game/Civilization/Data/DT_UnitBaseStat.DT_UnitBaseStat")).TryLoad());
	for (const FName& RowName : TechData.UnlockUnits)
	{
		FString DisplayName = RowName.ToString();
		if (UnitTable)
		{
			if (FUnitBaseStat* Row = UnitTable->FindRow<FUnitBaseStat>(RowName, TEXT("BuildUnlockInfoString")))
			{
				if (!Row->UnitName.IsEmpty())
				{
					DisplayName = Row->UnitName.ToString();
				}
			}
		}
		Lines.Add(FString::Printf(TEXT("ユニット 追加:\n +%s"), *DisplayName));
	}
