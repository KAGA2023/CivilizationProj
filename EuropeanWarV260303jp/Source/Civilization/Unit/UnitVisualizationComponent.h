...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "Combat Visualization")
    void OnCombatNotify_HitOnly(); // 중간 칼 휘두르는 타이밍용, Hit만 재생 (연타 연출)

    /** 원거리 전투 시 공격자 무기 SFXsocket → 방어자 메시 방향으로 화살 파티클 발사 (Notify_Hit / Notify_Combat에서 호출) */
    void SpawnRangedProjectile();

    /** 건설/수리/시설 제거 시 건설자 공격 몽타주 1회 재생. 재생 종료 시 InOnCompleted 호출. (이 컴포넌트의 Owner가 건설자) */
...
private:
    ...
    // 중간 Hit 전용 몽타주 재생 중 여부 (Notify_Hit으로 재생한 경우, 몽타주 종료 시 다음 단계로 진행하지 않음)
    bool bIsHitOnlyMontage = false;

    // 건설자 액션(공격 몽타주 1회) 재생 중 여부. 종료 시 OnBuilderActionMontageCompleted 호출
    ...
    // 도시 공격 시 Notify_Combat 타이밍에 재생할 Destroy 사운드 (캐시)
    UPROPERTY()
    USoundBase* CityAttackDestroySound = nullptr;

    /** 원거리 발사체 클래스 (에디터에서 지정 가능, 없으면 C++ 기본 클래스 사용) */
    ...
    // 방어자 Hit 몽타주만 재생 (Notify_Hit 연타용)
    void PlayDefenderHitMontageOnly();

    // 방어자 Hit 몽타주만 재생 (원거리, Notify_Hit 연타용)
