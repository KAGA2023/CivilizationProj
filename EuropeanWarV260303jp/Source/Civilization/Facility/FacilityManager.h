...
public:
    ...
    // 건설 스팀 이펙트 스폰 (몽타주 시작 시 호출용, 타일 좌표 기준)
    UFUNCTION(BlueprintCallable, Category = "Facility Management")
...
private:
	// 약탈 시설 위치별 연기 파티클 세트 (타일당 3개, 수리 시 제거, UPROPERTY 미사용: TMap value로 TArray 사용 시 리플렉션 미지원)
	...
	// 해당 월드 위치에 약탈 연기 파티클 스폰 후 맵에 등록
	...
	// 건설 시작 시 해당 위치 중점에 스팀 파티클 5개 스폰 (재생 후 자동 소멸)
