...
void ULoadingUI::GenerateWorldAsync()
{
	...
	if (bIsLoadMode && GameInst && GameInst->GetSaveLoadManager())
	{
		...
		if (SaveGameObject)
		{
			const FGameSaveData& SaveData = SaveGameObject->SaveData;
			Settings = SaveData.WorldConfig;
			// 인게임 레벨 진입 전에 GameInstance에 플레이어 수·국가 반영 (CreateAllPlayerStates가 올바른 수/국가로 생성하도록)
			GameInst->SetWorldConfig(SaveData.WorldConfig);
			...
			GameInst->SetCountryNames(RestoredCountryNames);
...
void ULoadingUI::NativeTick(const FGeometry& MyGeometry, float DeltaTime)
{
	...
	if (curPercent >= 0.7f && !bLevelTransitionStarted)
	{
		...
		if (USuperGameInstance* GameInst = Cast<USuperGameInstance>(GetWorld()->GetGameInstance()))
		{
