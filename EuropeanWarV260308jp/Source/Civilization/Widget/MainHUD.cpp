		...
		if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(GetWorld()->GetGameInstance()))
		{
			...
			if (FromState && ToState)
			{
				...
				case EDiplomacyActionType::SendGift:
					Message = FString::Printf(TEXT("%sが%sと友好会談を行いました"), *FromName, *ToName);
					break;
				case EDiplomacyActionType::Denounce:
					Message = FString::Printf(TEXT("%sが%sを非難しました"), *FromName, *ToName);
					break;
				case EDiplomacyActionType::DeclareWar:
					Message = FString::Printf(TEXT("%sが%sに戦争を宣言しました"), *FromName, *ToName);
					break;
				default:
					break;
				}
		...
		if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(GetWorld()->GetGameInstance()))
		{
			...
			if (FromState && ToState)
			{
				...
				case EDiplomacyActionType::OfferPeace:
					if (bAccepted)
					{
						Message = FString::Printf(TEXT("%sと%sが平和協定を結びました"), *FromName, *ToName);
					}
					break;
				case EDiplomacyActionType::OfferAlliance:
					if (bAccepted)
					{
						Message = FString::Printf(TEXT("%sと%sが同盟を結びました"), *FromName, *ToName);
					}
					break;
				default:
					break;
		...
		if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(GetWorld()->GetGameInstance()))
		{
			...
			if (StateA && StateB)
			{
				...
				case EDiplomacyStatusType::War:
					Message = FString::Printf(TEXT("%sと%sが戦争状態になりました"), *StateA->CountryName, *StateB->CountryName);
					break;
				case EDiplomacyStatusType::Peace:
					Message = FString::Printf(TEXT("%sと%sが平和協定を結びました"), *StateA->CountryName, *StateB->CountryName);
					break;
				case EDiplomacyStatusType::Alliance:
					Message = FString::Printf(TEXT("%sと%sが同盟を結びました"), *StateA->CountryName, *StateB->CountryName);
					break;
				default:
					break;
				}
...
void UMainHUD::OnAIPlayerDefeated(int32 DefeatedPlayerIndex)
{
	...
	if (LogUIWidget)
	{
		if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(GetWorld()->GetGameInstance()))
		{
			...
			{
				FString Message = DefeatedState->CountryName + TEXT("が敗北しました");
				LogUIWidget->AppendLine(Message);
			}
		}
	}
