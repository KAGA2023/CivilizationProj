	...
	if (ButtonPressed == EKeys::LeftMouseButton)
	{
		...
		if (TileData && GetWorld())
		{
			if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(GetWorld()->GetGameInstance()))
			{
				...
				{
					// 도시가 있고, 클릭된 타일 좌표가 도시 좌표와 일치하는지 확인
					if (PlayerState->HasCity())
					{
						FVector2D ClickedTilePos = TileData->GetGridPosition();
						FVector2D CityPos = PlayerState->GetCityCoordinate();
						
						if (ClickedTilePos == CityPos)
						{
		...
		// 건설자 유닛이 있으면서, 해당 타일이 플레이어(0) 소유이고, 도시 타일이 아닌 경우에만 브로드캐스트
		if (TileData && GetWorld())
		{
			if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(GetWorld()->GetGameInstance()))
			{
				if (UUnitManager* UnitManager = SuperGameInst->GetUnitManager())
				{
					...
					const bool bIsOwnedByPlayer0 = PlayerState0 && PlayerState0->IsTileOwned(HexPos);
					const bool bIsCityTile = WorldComp && WorldComp->IsCityAtHex(HexPos);
					if (UnitAtTile && bIsOwnedByPlayer0 && !bIsCityTile)
					{
