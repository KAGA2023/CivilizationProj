...
void ASuperPlayerState::UpdateAvailableFacilities()
{
    ...
    TArray<FName> AllRowNames = FacilityManager->FacilityDataTable->GetRowNames();

    // 플레이어 보유 기술 확인하여 필터링
    // TODO: 기술 시스템 구현 시 RequiredTechnology 확인하여 필터링
    // 현재는 모든 시설을 추가 (기술 시스템이 없으므로)
    for (const FName& RowName : AllRowNames)
    {
        FFacilityData* FacilityData = FacilityManager->FacilityDataTable->FindRow<FFacilityData>(RowName, TEXT("UpdateAvailableFacilities"));
        if (!FacilityData)
        {
            continue;
        }

        // 기술 조건 확인 (현재는 기술 시스템이 없으므로 모든 시설 추가)
        // TODO: 기술 시스템 구현 시 RequiredTechnology 확인
        // if (FacilityData->RequiredTechnology == NAME_None || HasTechnology(FacilityData->RequiredTechnology))
        // {
