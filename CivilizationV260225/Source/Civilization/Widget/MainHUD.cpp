		...
		if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(GetWorld()->GetGameInstance()))
		{
			...
			if (FromState && ToState)
			{
				...
				case EDiplomacyActionType::SendGift:
					Message = FString::Printf(TEXT("%s가 %s에게 외교 선물을 보냈습니다!"), *FromName, *ToName);
					break;
				case EDiplomacyActionType::Denounce:
					Message = FString::Printf(TEXT("%s가 %s을 비난했습니다!"), *FromName, *ToName);
					break;
		...
		if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(GetWorld()->GetGameInstance()))
		{
			...
			if (FromState && ToState)
			{
				...
				case EDiplomacyActionType::OfferPeace:
					if (bAccepted)
					{
						Message = FString::Printf(TEXT("%s와 %s이 평화협정을 맺었습니다!"), *FromName, *ToName);
					}
					break;
				case EDiplomacyActionType::OfferAlliance:
					if (bAccepted)
					{
						Message = FString::Printf(TEXT("%s와 %s이 동맹협정을 맺었습니다!"), *FromName, *ToName);
					}
					break;
				default:
					break;
...
void UMainHUD::OnAIPlayerDefeated(int32 DefeatedPlayerIndex)
{
	...
	if (LogUIWidget)
	{
		if (USuperGameInstance* SuperGameInst = Cast<USuperGameInstance>(GetWorld()->GetGameInstance()))
		{
			...
			{
				FString Message = DefeatedState->CountryName + TEXT("가 패망했습니다");
				LogUIWidget->AppendLine(Message);
			}
		}
	}
