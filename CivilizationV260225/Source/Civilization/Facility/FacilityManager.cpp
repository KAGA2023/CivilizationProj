...
void UFacilityManager::SpawnPillagedSmokeAtLocation(FVector WorldLocation, FVector2D TileCoordinate)
{
	...
	{
		PillagedSmokeTemplate = LoadObject<UParticleSystem>(nullptr, TEXT("/Game/StarterContent/Particles/P_Smoke.P_Smoke"));
	}
	if (!PillagedSmokeTemplate)
	{
		return;
...
void UFacilityManager::SpawnBuildSteamAtLocation(FVector WorldLocation)
{
	...
	{
		BuildSteamTemplate = LoadObject<UParticleSystem>(nullptr, TEXT("/Game/StarterContent/Particles/P_Steam_Lit.P_Steam_Lit"));
	}
	if (!BuildSteamTemplate)
	{
		return;
